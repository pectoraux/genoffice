...
Task ID: EXCEL-verification-debt-closure
Agent: Z.ai (Implementation / Verification Operator)
Task: Close the verification debt for EXCEL-018/020/021/022/023 per the architect's campaign directive — independent rerun of every evidence class on current main, defect fixes if any, deployed verification against production, and evidence strong enough for the architect's VERIFIED decisions. EXCEL-024 remains hard-stopped.

Work Log:

- Repository truth gate: origin/main advanced during the session (0616f75 PROJECT-023, then b336866 PROJECT-024 — both zero Excel overlap; local main fast-forwarded, Excel content identical to ef9ffb61e). All five implementations confirmed IN main: EXCEL-018 correction 82a87c9 (direct push, no PR), EXCEL-020 PR #12 (bf70fe9), EXCEL-021 PR #16 (e3d4311), EXCEL-022 PR #20 (cb1fff6), EXCEL-023 PR #23 (ef9ffb6).
- Found one uncommitted in-flight artifact: EXCEL-020 "test 6: lock-through-ribbon" (the Lock Cell direction the architect demanded). Ran it: PASSES through the full ribbon → journal → save → styles.xml → sheet protection → reopen chain. Committed in this evidence PR.
- EXCEL-018: 15-point source audit re-confirmed (pure dedupe, selection-scoped key, header semantics, descending deletes, no setValues, no-op + undersized fail-closed); dedupe unit 27/27; gateway structural 74/74; architecture guards 9/9; E2E 4/4 (both mandatory regressions: B7=B6 → B5=B4 as FORMULA; $D$6/$A6/A$6/A6 → row-4 refs with $ markers preserved). NEW independent byte proof xlsx-dedupe-byte-proof.test.ts (3 tests, package-level, no browser): survivor <c> records remain <f> formulas, s= indices travel with survivor rows, unrelated rows/cells and unrelated package parts byte-identical, zero-op no-op byte-identical worksheet. Deployed E2E vs production 4/4.
- EXCEL-020: gateway roundtrip 22/22, route validation 12/12, architecture guards 7/7, E2E 6/6 (unlock test 4 + NEW lock test 6; password fail-closed test 5: refusal, clean journal, verbatim password XML; toggle-back no-op test 3). Read-path source audit: hasPassword covers legacy + modern forms at both levels; SheetProtectionError fail-closed at sheet/workbook/protected-ranges. Deployed E2E vs production 6/6.
- EXCEL-021: gateway roundtrip 17/17, route validation 14/14, architecture guards 9/9, E2E 6/6 (import + file-native refusal, create → part wiring → reopen, session-delete no-op bytes, structural growth + header fail-closed, table-owned filter refusal, byte-for-byte preservation). Deployed E2E vs production 6/6.
- EXCEL-022: public-API audit clean (zero _image reaches; toBuilder/buildAsync only; remaining as-unknown-as casts are documented public-facade type-narrowing outside the image layer). Gateway image 20/20, route validation 19/19, architecture guards green, E2E 13/13 (incl. no-op byte-for-byte, delete cascades, one-cell/absolute fail-closed). Deployed E2E vs production 13/13.
- EXCEL-023: gateway chart 91/91, route validation 18/18, architecture guards 6/6, E2E 9/9 (15 scenarios + fail-closed proofs; dirty-state without cell-edit crutch; delete cascade with unrelated-chart survival; no-op byte preservation). Deployed E2E vs production 9/9.
- Cross-feature regression: FULL browser E2E 112/112 through the real HTTP stack (incl. formula+sort gate, formula+dedupe, table+filter, protection+style, chart/image structural shifts at gateway level). Forbidden-pattern scans clean: no JSZip / no OOXML construction / no value gateway imports / no Electron-Node / one canonical save pipeline / frozen surfaces untouched.
- Local gates on the evidence tree: typecheck clean (web, contractor-core, xlsx-gateway); format:check (CI-exact, FORMAT_BASE_REF=origin/main) clean; production build green (21.2s); unit suites web 223/223, xlsx-gateway 624/624 (incl. the 3 new byte-proof tests), contractor-core 441 passed + 4 skipped (pre-existing), web-host 78/78.
- CI: the canonical `web` gate is GREEN on the Excel merge commits and on main (runs 33072749904 on ef9ffb61e and 33082952617 on 0616f75 both show web=success; test=failure ONLY at the Lint step with 20 annotations ALL in frozen non-Excel files — re-verified via the check-run annotations API; e2e=failure ONLY at the frozen Electron shell step).
- Production deployment: Vercel chain verified — production deployments exist for every Excel merge (ef9ffb61e EXCEL-023, cb1fff60f EXCEL-022, and the full chain down to bf70fe9-era); current production = b3368664d (Excel content identical to ef9ffb61e). DEPLOYED evidence executed against https://genoffice.vercel.app: 57/57 browser E2E green (38 feature: dedupe 4 + protection 6 + tables 6 + images 13 + charts 9; plus 19 core: browser/formula/structural/data/filter) with actual downloaded-XLSX byte inspection inside the suites.
- Defects discovered requiring recode: NONE. All five implementations passed every evidence class on independent rerun. The only gaps were missing EVIDENCE artifacts (the lock-direction E2E test and the package-level dedupe byte proof), both supplied in this PR.
- Documentation updated per policy AFTER evidence completion: spec/excel/work-items.md (five sections: factual merged statuses + verification-evidence maps; NO VERIFIED markings), spec/excel/verification-matrix.md (EXCEL-022 post-merge steps marked complete).

Stage Summary:

- All five items EXCEL-018/020/021/022/023: UNIT, ARCH, GOLDEN (byte), BROWSER, DEPLOYED, CI(web gate) all green on independent rerun. Recommendations: READY FOR ARCHITECT VERIFICATION each (architect owns the VERIFIED transitions). No EXCEL-024 work performed — hard stop honored.

---

Task ID: EXCEL-024-impl-phase-a
Agent: Z.ai (Implementation Operator)
Task: EXCEL-024 (Conditional Formatting) — repository-truth gate + Phase A reconfirmation at current main, then implementation on branch excel-024-cf.

Work Log:

- Environment was reset again (/home/z/genoffice gone); re-cloned pectoraux/genoffice (public). origin/main = 4d2ec337 "docs(excel): record architect verification for EXCEL-018 through EXCEL-023". Worktree clean. Spec ledger verified: EXCEL-024 defined in work-items.md (no status) + verification-matrix.md. No excel-024* branch on origin (ls-remote); PR #25 (evidence) MERGED (65c7ba4 in ancestry); spec/excel/verification-decision-2026-08-27.md = architect decision record: EXCEL-018..023 ALL VERIFIED, "EXCEL-024 ... UNBLOCKED ... may proceed to implementation". All six implementation SHAs verified in main ancestry. Branch excel-024-cf created from 4d2ec337. npm ci clean.
- Phase A reconfirmed from source at HEAD (audit held; every point re-verified):
  - WRITE path complete: xlsx-cf.ts (501 lines) — CfWireRule{ranges,stopIfTrue,rule} (Univer wire shape), applyCfRules() with x14-linked-block preservation + byte-identical probe + CfEditError fail-closed, cfRuleUnsaveableReason() dry-run, iconSetSaveable(), OOXML_ICON_SETS (16), WORST_FIRST_ICON_SETS, buildDxfXml (Univer IStyleBase → dxf). SheetCfState exists (xlsx-gateway.ts:169). applyCellEditsToXlsx accepts cfStates (8th positional, :764), applied AFTER structural replay + cell edits + hyperlinks, BEFORE dvStates (:1163-1167); stylesheet loaded when cfStates non-empty.
  - READ path: NONE (WorksheetState ends at charts/sheetProtection — no cfRules). readBasicWorkbook parses DV/notes/tables/etc but not <conditionalFormatting>.
  - ROUTE: office-routes.ts hardcodes [] [] at hyperlinkEdits/cfStates positions (:3965-3966); BrowserWorkbookSavePlan has no cfStates field. Open route returns gateway snapshot verbatim → adding WorksheetState.cfRules auto-extends the open wire.
  - STRUCTURAL: gateway transformRangedFeatures shifts CF sqref (xlsx-structure.ts:1093, fully-deleted range removes the element); transformFormulas shifts <formula> bodies (regex formula[12]? covers cfRule <formula>) via shiftFormulaText.
  - DESKTOP reference (read-only): applyConditionalRules (priority-asc install under journalSuppression, per-rule best-effort), buildConditionalRule (colorScale/dataBar incl. x14 negativeColor/iconSet with WORST_FIRST inversion + ascending→descending conversion), buildHighlightCondition (NO aboveAverage, NO containsErrors, NO timePeriod — the silent-skip gaps; web must parse aboveAverage + errors, fail closed on timePeriod), CELLIS_FORMULA_CELL_LIMIT blank-divergence compensation, collectCfStates (declarative full-rule-set snapshot of cfDirty sheets), CF_RULE_COMMAND_PATTERN up-front block via iconSetSaveable + cfRuleUnsaveableReason (event.cancel), desktop does NOT shift live CF ranges on structural ops (stale live model; file stays correct via gateway save).
  - UNIVER 0.25.1 verified from installed node_modules: mutation IDs sheet.mutation.{add,set,delete,move}-conditional-rule; panel op sheet.operation.open.conditional.formatting.panel {value:2}; FWorksheet.newConditionalFormattingRule()/addConditionalFormattingRule()/getConditionalFormattingRules(); IConditionFormattingRule{ranges,cfId,stopIfTrue,rule}; rule configs (highlightCell subTypes number[8 ops]/text[10 ops]/formula/rank/average/duplicateValues/uniqueValues, colorScale, dataBar{min,max,isGradient,positiveColor,nativeColor}, iconSet{operator,value,iconType,iconId}[]); IStyleBase{bl,it,st:{s},ul:{s},cl:{rgb},bg:{rgb},n:{pattern},...}; CFValueType{num,min,max,percent,percentile,formula}. CONFIRMED GAP: CF package structural handler (es/index.js:2713-2750) only markRuleDirty — NO range transform anywhere → live CF ranges go stale; browser-side shift REQUIRED (§8).
  - moveRange canonical semantics captured (xlsx-structure.ts:780-812): insert {start>=b?+d, end>=b?+d}; delete clamps (start in span→ds, end in span→ds-1, fully-inside→null). Browser shift helper must mirror THIS, not shiftIndex (point semantics differ on boundary clamp).
  - set-conditional-rule mutation params {unitId,subUnitId,cfId?,rule}; delete {unitId,subUnitId,cfId}; add {unitId,subUnitId,rule}.
  - Route DV validation precedent (expectDvWireRule:2289+) + DV read precedent (parseDataValidations fail-closed per sheet, DvReadError → dvRules omitted, workbook opens).
  - Architecture guard (tests/architecture.test.ts:192): apps/web/src value imports of @genoffice/xlsx-gateway barrel BANNED (import type only). Up-front unsaveable check must NOT import gateway values → browser-side structural pre-check NOT used; route-side §11 validation + writer fail-closed carry the enforcement; browser lock uses only Univer APIs.
- Design decisions (per architect §3-§12): WorksheetState gains cfRules?: CfWireRule[] + cfLocked?: boolean (sheet carries CF the model cannot represent → browser refuses CF mutations on it — prevents the silent-loss path the desktop has with timePeriod); reader emits Univer wire shape with dxf style PRE-RESOLVED into rule.style; x14-linked blocks (extLst) → per-sheet CfReadError (DV precedent, §6 example); timePeriod → per-sheet CfReadError; browser install under journalSuppression via add-conditional-rule mutations; cfDirty journal on the 4 CF mutations; save = collectCfStates declarative snapshot; structural-op live-range shift under suppression via set/delete-conditional-rule mirroring moveRange; ribbon Home → Styles group → Conditional Formatting button invoking the real panel op.

Stage Summary:

- Repository-truth gate PASSED; Phase A reconfirmed with zero contradictions to the prior audit plus exact canonical moveRange semantics and mutation param shapes captured. Implementation begins: gateway CF reader (xlsx-cf.ts read side + dxfs exposure in StylesheetReader) → WorksheetState.cfRules/cfLocked → route cfStates validation + pass-through → web import/install/lock/journal/shift/collect → ribbon entry → fixtures/tests/E2E.

---

Task ID: EXCEL-024-impl-completion
Agent: Z.ai (Implementation Operator)
Task: Continue EXCEL-024 (Conditional Formatting) to completion — the prior session died mid-implementation with ~2,100 lines uncommitted on excel-024-cf; audit everything, re-verify every gate independently, correct defects, finalize documentation, commit, rebase, push.

Work Log:

- Context recovery per the never-trust-prior-reports rule: repo truth gate re-run first. origin/main had advanced 4d2ec337 → 3b8b2a4 (PROJECT-025 calendar + project-reconciliation merges; ZERO file overlap with the EXCEL-024 set). Local branch excel-024-cf sat at 4d2ec337 with the prior session's uncommitted work: 14 modified files + 3 new test files (gateway reader +754 lines, route validation +544, ExcelEditor +241, fixtures +282, E2E spec 861 lines, architecture guards +107).
- Full source audit of the uncommitted diff: gateway read path (parseConditionalFormatting = exact inverse of applyCfRules, dxf pre-resolution through StylesheetReader.dxfAt, per-sheet CfReadError fail-closed → cfRules omitted + cfLocked), WorksheetState.cfRules/cfLocked additive fields, route cfStates strict validation (§11: unknown-field rejection at every level, enum mirrors, caps, OOXML_ICON_SETS imported from the gateway barrel as the single icon-set source), position-8 cfStates pass-through into applyCellEditsToXlsx, browser install/journal/collectCfStates/ribbon wiring. All consistent with the architect's §3–§12 directives.
- CRITICAL §8 verification (the prior session's central claim, untested when it died): "Univer 0.25.1 natively transforms live CF rule ranges on structural ops." Three independent probes:
  (a) Source analysis of the base @univerjs/sheets-conditional-formatting package: NO RefRange registration anywhere — the structural handler only markRuleDirty (the Phase A finding, correct but incomplete).
  (b) Headless Node probe (scripts/cf-range-shift-probe.mjs, full plugin set minus render-dependent CF-UI): install rule B2:B6 → insertRows(1,2) → ranges UNCHANGED, ZERO CF mutations fired. Without CF-UI, ranges go stale.
  (c) Source analysis of @univerjs/sheets-conditional-formatting-ui (registered by the browser preset): ConditionalFormattingFormulaRefRangeController registers EVERY rule's ranges+formulas with sheets-formula's FormulaRefRangeService; on structural ops the controller emits set/add/delete-conditional-rule side-effect mutations that rewrite live ranges (extend-at-top-edge on insert, shrink on delete, formula-rule splits) — and those mutations flow into the browser CF-dirty journal.
  (d) Real-browser proof: E2E tests 5/6/10 (row insert extends B2:B6→B2:B8 + persists + reopens; column insert extends/shifts; row delete shrinks B2:B6→B2:B4) all PASS through the real browser → Vite → HTTP → routeOffice → gateway chain.
  CONCLUSION: the prior session's §8 resolution is CORRECT (native transform, no browser-side shift — a shift would double-transform), but its mechanism description ("the sheets RefRange service") was imprecise; corrected in ExcelEditor.tsx, the E2E spec, the architecture guard, and the spec ledger to name the CF-UI controller + FormulaRefRangeService precisely, including the boundary note (Univer extends at the top edge — Excel's behavior; the gateway's moveRange shifts; the divergence is inert because any native transform marks the sheet CF-dirty and applyCfRules replaces the whole CF set with the live snapshot AFTER structural replay).
- Boundary/ordering audits: add-conditional-rule PREPENDS (unshift — verified at CF package line 332) → the priority-descending install leaves the most-important rule at the model front; the writer assigns sequential priorities 1..n in array order → round-trip stable. Load-time install sits inside the journalSuppression window (CommandExecuted handler early-returns on suppressionRef) → opening never dirties. cfStates apply position verified against the applyCellEditsToXlsx signature (8th positional, after hyperlinkEdits, before dvStates).
- Independent gate re-runs (nothing trusted from the dead session's spec-ledger claims): typecheck clean (xlsx-gateway, contractor-core, web); unit — xlsx-gateway 668/668 (44 new CF reader tests incl. §16 byte proofs: no-op preservation, unrelated-part byte identity, structural sqref+formula shifting, write→reopen round-trip), contractor-core 458 passed + 4 skipped (17 new cfStates validation tests), web 229/229 (6 new architecture guards); ESLint clean on every changed file; FORMAT_BASE_REF=origin/main format:check clean; production build green (19.9s); FULL browser E2E 122/122 re-run in 8 chunks (CF spec 10/10, core excel 15/15, shell+insert 16/16, charts+DV+data 20/20, images+filter 18/18, persistence+protection+dedupe 13/13, notes+table+view 14/14, word 16/16 — 106 Excel + 16 Word).
- Defects found and fixed: (a) spec ledger test-count error — prior session claimed "full browser E2E 123/123 (106 Excel + 17 Word)"; the tree actually carries 122 tests (16 Word); corrected with the re-verified count. (b) Imprecise §8 mechanism descriptions in four files (see above). No implementation defects found — the uncommitted code passed every gate on independent re-run.
- Documentation finalized per policy (after evidence): spec/excel/work-items.md EXCEL-024 evidence block corrected; spec/excel/verification-matrix.md EXCEL-024 section gained the full evidence map (UNIT/ARCH/GOLDEN-BYTE/BROWSER green; DEPLOYED explicitly NOT YET — push blocked).
- Push attempt: `git push --dry-run` fails — no GitHub credentials exist in this reset environment (no ~/.bashrc, no GITHUB_TOKEN in env, no gh CLI, no credential helper; prior sessions' PR scripts read GITHUB_TOKEN from the environment). Branch remains local; push + PR + CI + DEPLOYED evidence resume the moment credentials are restored.

Stage Summary:

- EXCEL-024 implementation COMPLETE and committed on branch excel-024-cf (gateway CF reader + dxf resolution, cfRules/cfLocked wire, strict cfStates route validation, browser install/journal/fail-closed gate/declarative snapshot/ribbon entry, Univer-native structural transforms with the CF-dirty journal carrying them into the save). All local gates green on independent re-run: typecheck ×3, unit 668/668 + 458+4sk + 229/229, architecture 6 new guards, format CI-exact, ESLint, production build, full browser E2E 122/122.
...
Task ID: EXCEL-025-phase-a
Agent: Z.ai (Implementation Operator)
Task: EXCEL-025 (Named Ranges / Name Manager) — repository-truth gate + Phase A forensic audit before implementation.

Work Log:

- Repository truth: environment reset again (repo + ~/.bashrc gone) — PAT re-persisted from the standing instruction, repo re-cloned. origin/main = 71188d9 "Merge EXCEL-024 Conditional Formatting" (architect merged PR #29; ledger still says PR_OPEN — the merge is repo fact, VERIFIED remains the architect's call). EXCEL-018–024 all in main ancestry; EXCEL-015 dependency VERIFIED; EXCEL-025 defined in work-items/verification-matrix. Branch excel-025-named-ranges created from 71188d9. npm ci clean.
- CANONICAL WRITER ALREADY EXISTS (xlsx-gateway/xlsx-defined-names.ts, 126 lines): applyDefinedNamesState(workbookXml, DefinedNamesState{names: DefinedNameEntry{name, formula, sheetIndex?}[], preserveNames: string[]}). Semantics: drops every modeled entry from <definedNames>, keeps _xlnm.*/hidden/preserve-listed byte-verbatim, appends additions into the existing section (or inserts a new one after </sheets>/externalReferences). validateName: Unicode NAME_PATTERN, cell-ref lookalikes, true/false, _xlnm prefix, 1..255; per-(name,scope) duplicate + modeled∩preserved collision → DefinedNameError (fail-closed save). applyCellEditsToXlsx accepts definedNamesState at positional 11; plan-level fail-closed guard rejects names+structuralOps/sheetPlan in one save ("save one of them first"); ordering: applyDefinedNamesState runs after sheetPlan, applyPrintAreas AFTER it (page-setup family owns _xlnm.Print_Area/Print_Titles; the names rewrite keeps them verbatim).
- Gateway XML-level interplay already complete: renameSheetReferencesInDefinedNames (sheet rename), localSheetId positional remap + scoped-name drop + definedNamesReferenceSheet removal guard (sheet delete/reorder), shiftDefinedNames (structural row/col shifts rewrite bodies).
- READ PATH: NONE — WorkbookSnapshot{revision, sheets, workbookProtection} has no names field; handleOpenWorkbook returns the snapshot verbatim (adding a field auto-extends the open wire, the EXCEL-024 cfRules precedent). Route hardcodes null at positional 11. Web save plan has no definedNamesState. Ribbon Formulas → Defined Names → Name Manager button exists but DISABLED ("the web save plan does not yet expose definedNamesState"). Web NameBox exists; goTo validates A1/range only.
- DESKTOP REFERENCE (read-only audit): reader (Rust sidecar read_defined_names) filters _xlnm/hidden/empty-name/empty-formula. applyDefinedNames installs grouped+ranked (workbook-scope 0 → sheet-scope 1 → #REF! residues 2) under journalSuppression via newDefinedNameBuilder().load({name, formulaOrRefString, localSheetId: sheetId ?? 'AllDefaultWorkbook'}).build() + insertDefinedNameBuilder; per-entry catch → uninstalledDefinedNames (save preserves verbatim). collectDefinedNamesState: null when not dirty; full live model; localSheetId → file sheet-order index (throws on unknown sheet); preserveNames = uninstalled. Journal: DEFINED_NAME_MUTATIONS = {formula.mutation.set-defined-name, formula.mutation.remove-defined-name} → single workbook-level names-dirty flag. Name Manager dialog: list/add/edit(name+ref)/delete, scope at creation only. Name Box: resolveGoToRef — names win over addresses (case-insensitive), jump only when the ref is a plain A1 reference. Save: splitSave holds names when structural/sheet ops present (phase 1 structure, phase 2 names alone); post-save openLazyWorkbook + applyDefinedNames re-syncs the live model.
- UNIVER 0.25.1 (verified from installed node_modules, not docs): mutations formula.mutation.{set,remove}-defined-name (engine-formula). FWorkbook facade: getDefinedNames(), newDefinedNameBuilder(), insertDefinedNameBuilder(). FDefinedNameBuilder: setName/setFormula/setRef/setRefByRange/setComment/setScopeToWorksheet/setScopeToWorkbook/setHidden/build()/load(); build() runs validateDefinedName → THROWS on empty name, duplicate name (name-keyed first-match, id-exempt), table-name conflict, reference-lookalike, invalid start (CJK exempt), sheet-name conflict, empty formula, function-name conflict. FDefinedName: getName/setName/setFormula/setRef/getFormulaOrRefString/setScopeToWorksheet/setScopeToWorkbook/setHidden/delete/getLocalSheetId/isWorkbookScope/toBuilder. Scope sentinel 'AllDefaultWorkbook'; sheet scope carries the Univer sheet id. DECISIVE §8-equivalent finding: UpdateDefinedNameController (sheets-formula, registered by the web's preset-sheets-core) intercepts EVERY structural command + SetWorksheetNameCommand + RemoveSheetCommand + name-on-name refs and rewrites live name formulaOrRefStrings, emitting set-defined-name mutations — the live model stays in sync AND the journal sees names-dirty (→ split-save when structural ops ride the same save). SetDefinedNameCommand itself does NOT validate (validation lives in the builder). DefinedNamesService is id-keyed; getValueByName is case-insensitive first-match.
- Web wiring facts: preset-sheets-core includes UniverSheetsFormulaPlugin (names resolve in cell formulas); snapshot sheets become Univer sheets with id = sheet.id in tab order (sheetIndex ↔ snapshot.sheets[i].id is the scope mapping, desktop parity); the web already has a split-save precedent (heldTables, EXCEL-021) to mirror for names; the journal listener + journalSuppressionRef pattern (CF/DV/notes) extends with the two name mutations.

Stage Summary:

- Architecture (smallest canonical boundary): XLSX → gateway reader parseDefinedNamesState → WorkbookSnapshot.definedNames{names, preserveNames} (+ namesLocked fail-closed marker when the section cannot be parsed) → browser install (desktop rank order, builder API, journal suppression, per-entry catch → runtime preserve set) → live Univer name model (Name Manager + Name Box resolution) → namesDirty journal on the two mutations → collectDefinedNamesState (FULL live model + reader preserve ∪ runtime rejects) → BrowserWorkbookSavePlan.definedNamesState → route strict validation (writer-mirrored name rules, scope bounds, caps, unknown fields, collision → 400 before bytes) → applyCellEditsToXlsx(…, definedNamesState) → applyDefinedNamesState → XLSX. Split-save (desktop heldNames parity) when structuralOps present.
- Fail-closed classification: (a) preserved, not editable, auto-kept by writer — _xlnm.* built-ins (Print_Area/Print_Titles owned by page-setup), hidden names; (b) preserved via reader classification, never modeled — invalid names (writer-pattern mirror), localSheetId out of range, empty formula/body, malformed elements; (c) preserved via runtime install-reject — duplicate-name losers (ranked; case-insensitive), table/sheet/function name conflicts; (d) namesLocked — unparseable section refuses all Name Manager edits, no-op save preserves bytes; (e) save-side fail-closed — names+structuralOps same save → split-save; modeled∩preserved collision → DefinedNameError (desktop parity).
- THE critical preservation invariant (architect §6): applyDefinedNamesState's keep-rules + preserveNames + the declarative full-model snapshot mean editing one supported name never silently deletes or rewrites unrelated names — reader classification feeds preserveNames directly, and the writer only ever drops entries it is about to re-add from the model.

---

Task ID: EXCEL-025-impl
Agent: Z.ai (Implementation Operator)
Task: EXCEL-025 (Named Ranges / Name Manager) — full implementation on branch excel-025-named-ranges per the architect's directive: repository-truth gate, Phase A forensic audit, implementation, all local gates, PR.

Work Log:

- Repository truth: environment reset again (repo + ~/.bashrc gone) — PAT re-persisted from the standing instruction, repo re-cloned. origin/main = 71188d9 "Merge EXCEL-024 Conditional Formatting" (the architect merged PR #29). EXCEL-018–024 all confirmed in main ancestry; EXCEL-015 dependency VERIFIED. Branch excel-025-named-ranges created from 71188d9. Phase A findings recorded in worklog (task EXCEL-025-phase-a) BEFORE any implementation.
- Implementation (canonical-first): gateway reader `parseDefinedNamesState` (the exact inverse of the pre-existing canonical writer `applyDefinedNamesState` — extended, never duplicated) + `definedNameIsSaveable` export (the single name predicate, reused by the route); `WorkbookSnapshot.definedNames {names, preserveNames}` + `namesLocked` fail-closed marker; `BrowserWorkbookSavePlan.definedNamesState` wire + `expectDefinedNamesState` strict route validation (unknown fields, canonical predicate, scope bounds, duplicates, collision → 400 before bytes) threaded into applyCellEditsToXlsx position 11 (previously null); browser install via the PUBLIC builder facade under journal suppression, names-dirty journal on formula.mutation.set/remove-defined-name, collectDefinedNamesState full-model snapshot + preserve union, split-save (heldNames — desktop parity) when structural ops ride the save, Name Manager dialog + Ribbon enablement + namesLocked gate, Name Box name resolution (desktop resolveGoToRef port) with goTo widened to Sheet!-prefixed/$-marked refs (desktop isA1Reference parity — the facade resolves the prefix and switches sheets).
- Defects found and fixed during E2E: (a) the Name Box's api.goTo pre-validation rejected every sheet-qualified and $-marked reference (parseAddress has no prefix/$ grammar) — the original Name Box E2E for 'GlobalTotal' passed VACUOUSLY (active cell was already A1); fixed by prefix-stripping + $-stripping for the syntax check while passing the original string to the facade (probe-verified: ws.getRange('Other!$B$2:$B$4') returns a range on sheet-2). (b) The first fixture carried the Excel_Version duplicate pair, so EVERY name edit's save failed the writer's modeled∩preserve collision guard — correct fail-closed behavior (exact desktop parity) but wrong fixture for the create/edit/delete flows; split into a dedicated collision fixture + test proving the save fails closed with the writer's exact message. (c) Test 7's save button was disabled (nothing dirty after a read-only open) — added a far-away plain cell edit to enable saving without touching the name or formula.
- All local gates green on the final tree: typecheck ×3 (xlsx-gateway, contractor-core, web); unit — xlsx-gateway 693/693 (25 new), contractor-core 475 passed + 4 skipped (17 new), web 235/235 (6 new architecture guards); ESLint clean on all 17 changed files; FORMAT_BASE_REF=origin/main format:check clean; production build green (19.7s); FULL browser E2E 133/133 in 7 chunks through the real stack (names 11/11 + CF 10/10 + core 15/15 + shell/insert/charts 25/25 + DV/data 11/11 + images/filter 18/18 + persistence/protection/dedupe 13/13 + notes/table/view/word 30/30); frozen surfaces (apps/sheets, apps/docs, apps/slides, apps/shell, packages/platform-electron, packages/renderer-bridge) zero changes.
- Documentation updated per policy AFTER evidence: spec/excel/work-items.md EXCEL-025 status + full evidence block; spec/excel/verification-matrix.md EXCEL-025 evidence map (UNIT/ARCH/GOLDEN-BYTE/BROWSER green; DEPLOYED explicitly NOT YET).
...
Task ID: EXCEL-025-blocker-audit
Agent: Z.ai (Implementation Operator)
Task: PR #31 architect blocker (CHANGES REQUESTED) — forensic audit of the scope-collapsing reader defect before the correction.

Work Log:

- Blocker read from the PR #31 conversation (pectoraux, 2026-08-28T04:12:02Z): parseDefinedNamesState groups by entry.name.toLowerCase() ALONE, then ranks workbook→sheet→#REF! and puts the losers in preserveNames — a legitimate sheet-scoped definition sharing a name with a workbook-scoped one is silently lost from the model AND poisons the save (the writer's modeled∩preserve guard keys by name). Required: (case-insensitive name + scope) grouping, preserve only genuine same-scope duplicates/unmodelable entries, gateway + browser coverage for same-name-different-scope, prove both resolve/edit independently, rerun full gates.
- Defect verified in source (xlsx-defined-names.ts lines 242-259): byName keys on name.toLowerCase() only; rank workbook=0/sheet=1/#REF!=2; sorted[0] modeled, slice(1) → preserveNames. The canonical writer (line 46) keys uniqueness on name + (sheetIndex ?? -1) and the route (office-routes.ts line 3108) mirrors it — reader is the odd layer out. Codified in the read test 'ranks duplicate names the way the desktop installs them' and E2E test 11 (cross-scope pair treated as a collision, asserted as desktop parity).
- Engine facts re-verified from installed node_modules (@univerjs 0.25.1, not docs): DefinedNamesService map is ID-keyed ([unitId][id]) — it CAN hold two same-name entries; getValueByName/_nameCacheMap are name-keyed case-insensitive (last registration wins the cache). build() runs validateDefinedName whose duplicate check calls getValueByName — SCOPE-BLIND: the public builder rejects a second same-name entry at ANY scope (this is why the frozen desktop ordered+collapsed: its applyDefinedNames feeds workbook-level live definitions first). SetDefinedNameCommand does NOT validate; FDefinedName._apply (setName/setFormula/setRef) executes the command directly (no validation); delete() removes by id; getDefinedNames() returns the full id-keyed map; UpdateDefinedNameController iterates Object.values of the id-keyed map — structural shifts rewrite EVERY entry including same-name siblings.
- Consequence chain under the current reader: file with Total@workbook + Total@sheet0 → reader models only Total@workbook, preserveNames=['Total'] → ANY names-dirty save (even editing an unrelated name) hits route 400 / DefinedNameError 'also exists' — the architect's 'subsequent edits fail incorrectly'.
- Correction design (this audit's conclusions): (1) READER: group by (name.toLowerCase(), (sheetIndex ?? -1)) — the writer's exact uniqueness key, case-insensitive; single-entry groups model; multi-entry groups are GENUINE same-scope duplicates (Excel never writes them) → winner (live over #REF!, stable file order) modeled, losers preserved; a name already on the preserve list never carries a modeled entry (preserveNames is name-granular — the writer keeps every element with that name and rejects any modeled entry with it, so modeling one element of a poisoned name would make every save fail). (2) WRITER + ROUTE: duplicate keys become case-insensitive (Excel names are case-insensitive; 'Total'+'TOTAL' at one scope is a duplicate). (3) BROWSER install: same-name groups install the FIRST entry through the validated builder path; siblings (provably unique per (name, scope) in the canonical model) install through the same PUBLIC insertDefinedNameBuilder facade with a hand-filled ISetDefinedNameMutationParam (public type — the engine service is id-keyed and holds/lists/edits/deletes/structurally-shifts them per id); if the first entry's build() rejects, the whole name group goes to namesUninstalled (preserved). (4) Name Manager create/rename: scope-aware duplicate pre-check (same-scope case-insensitive → reject; different scope → allowed via the sibling path; name in preserve sets → reject). (5) Name Box resolution: scope-aware precedence — entry scoped to the ACTIVE sheet, then workbook-scoped, then any (desktop fallback parity). (6) E2E test 11 redesigned around a GENUINE same-scope duplicate (fail-closed coverage retained); new scenarios for the cross-scope pair (open/install/list, Name Box scope resolution, edit-one-preserves-other, delete-one, structural shift of both). Known limitation to document: Univer 0.25.1 formula-engine name resolution is name-keyed (scope-blind) — cell formulas referencing an ambiguous same-name resolve to the last-registered entry; the FILE keeps both scopes correct (canonical bytes are the contract); Name Box/Name Manager are scope-correct in the browser.

Stage Summary:

- Blocker root-caused with source-level evidence at all four layers (reader grouping, writer key, route key, engine validation). Correction design settled: (case-insensitive name + scope) is the single canonical uniqueness rule mirrored in reader grouping, route validation, and writer guard; the browser holds BOTH same-name entries in the engine's id-keyed model via the public facade; genuine same-scope duplicates remain fail-closed (winner modeled + losers preserved). Implementation next.

---

Task ID: EXCEL-025-blocker-correction
Agent: Z.ai (Implementation / Verification Operator)
Task: PR #31 architect blocker (CHANGES REQUESTED) — implement the required correction, re-run the full gates, push, and respond on the PR.

Work Log:

- Repository truth first: branch excel-025-named-ranges checked out clean @ 22272b7 (= PR #31 head); origin/main advanced 71188d9 → a13a667 (PROJECT-027 Electron shell, PR #30) with ZERO file overlap with PR #31. Blocker text read from the PR #31 conversation (pectoraux, 2026-08-28T04:12:02Z): reader groups by name alone, collapsing legitimate cross-scope definitions; writer keys uniqueness on (name, sheetIndex); required — (case-insensitive name + scope) grouping, preserve only genuine same-scope duplicates, gateway + browser coverage for same-name-different-scope, prove both resolve/edit independently, re-run full gates.
- Implementation (commit 753f13a, 9 files): (1) gateway reader parseDefinedNamesState groups on `${name.toLowerCase()}\u0000${sheetIndex ?? -1}` — the writer's exact key — cross-scope same names model BOTH; genuine same-scope duplicates (Excel never writes them) keep winner-modeled + losers-preserved (live over #REF!, stable file order). (2) New reader rule: names preserved for UNMODELABLE elements (dangling scope/empty body/unsaveable name) never carry a modeled entry at any scope (preserveNames is name-granular; a modeled sibling would fail every names-dirty save at the writer's collision guard) — tracked via a poisonedNames set so duplicate-group losers deliberately KEEP the architect-endorsed fail-closed combination. (3) Writer + route duplicate keys made case-insensitive ('Total'+'TOTAL' at one scope = duplicate; at different scopes = legal pair). (4) Browser install: case-insensitive name groups install the first entry through the validated builder; same-name siblings ride the PUBLIC insertDefinedNameBuilder facade with a hand-filled public ISetDefinedNameMutationParam (engine facts re-verified from installed source: DefinedNamesService is id-keyed; build()→validateDefinedName→getValueByName is name-keyed scope-blind; SetDefinedNameCommand/_apply/delete do NOT validate; UpdateDefinedNameController shifts the full id-keyed map); group rollback to file-only if the first entry's build() rejects. (5) handleDefinedNameAction: scope-aware create/rename validation (same-scope case-insensitive duplicate refused with a clear message; cross-scope create via the sibling path; preserve-set names refused). (6) Name Box resolveGoToNameRef: Excel scope precedence — active-sheet-scoped shadows workbook, then workbook, then any (desktop fallback parity).
- One mid-gate defect found and fixed: the first cut of the poisoned-name rule ALSO stripped the winner of a genuine same-scope duplicate (the exact fail-closed combination the architect endorsed and E2E 11 depends on); fixed by splitting poisonedNames (unmodelable per-element) from duplicate-group losers.
- Gates on the corrected tree: typecheck ×3 green; gateway 701/701 (+9: pair modeling ×3, genuine duplicate, poisoned name, pair round-trip, edit-one-preserves-twin byte-verbatim, delete-one-preserves-other both directions, case-insensitive writer rule); contractor-core 477 passed + 4 skipped (+2 case-rule wire tests); web 235/235 (architecture guards intact); FORMAT_BASE_REF=origin/main format:check clean (after prettier on 3 files); ESLint clean on all changed files (one prefer-const fixed); production build green (19.1s); FULL browser E2E 139/139 (names 17/17 — test 11 redesigned around a GENUINE same-scope duplicate + 6 new pair scenarios: open/install/list, Name Box scope precedence, edit-one-preserves-other both directions, delete-one-preserves-other, structural shift of both, create-at-new-scope + same-scope refusal; full-suite regression 122 in 5 chunks + 17 names); frozen surfaces zero changes (diff confined to the PR #31 file set: BOQ.tsx annotations proven NOT in the diff).
- Push + PR response: commit 753f13a pushed to PR #31; blocker-response comment posted (issuecomment 5448677433) mapping each of the architect's 5 required corrections to its evidence, including the documented engine limitation (Univer 0.25.1 formula-engine name resolution is name-keyed/scope-blind for AMBIGUOUS same-names in cell formulas — the file preserves both scopes; Name Box/Manager are scope-correct).
- CI on 753f13a driven to conclusion: web GREEN (the canonical Excel gate); test fails ONLY at Lint with 20 annotations file-for-file identical to the established pre-existing set (BOQ.tsx NOT in the diff — verified), zero in EXCEL-025 files; e2e fails ONLY at the frozen Electron shell step; foundation fails ONLY at Verify branch isolation (the structural cross-domain guard, identical on merged+verified PRs #23/#25/#29); desktop-e2e green. Ledger + verification matrix updated AFTER evidence (correction block + CI line at head 753f13a).

Stage Summary:

- PR #31 blocker fully addressed: the canonical (case-insensitive name, scope) uniqueness rule is now enforced identically at reader grouping, route validation, and writer guard; a same-name cross-scope pair models, installs, lists, resolves, edits, deletes, and structurally shifts as TWO independent definitions with byte-verbatim preservation of the untouched twin; genuine same-scope duplicates remain fail-closed. Workflow state: CHANGES ADDRESSED — PR #31 head 753f13a, web gate GREEN, awaiting the architect's re-review (ARCHITECT_REVIEW → APPROVED → MERGED → VERIFIED remain the architect's transitions). NOT VERIFIED.

---

Task ID: EXCEL-025-blocker2-correction
Agent: Z.ai (Implementation / Verification Operator)
Task: PR #31 second architect blocker (CHANGES STILL REQUIRED — case-variant same-scope duplicates) — implement the required correction, re-run the full gates, push, and respond on the PR.

Work Log:

- Repository truth first: branch excel-025-named-ranges checked out clean @ f852649 (= PR #31 head after the first blocker correction). Blocker read from the PR #31 review API (pectoraux, 2026-08-28T06:00:50Z, review id 5048312837): the reader groups case-variant duplicates case-insensitively (same-scope 'Foo'+'foo' → winner modeled, loser preserved), but applyDefinedNamesState and the route's expectDefinedNamesState compared preserveNames CASE-SENSITIVELY — a names-dirty save could emit BOTH variants instead of failing closed. Required: case-insensitive modeled∩preserve collision check at the canonical writer boundary, mirrored in route validation, regression proving case-variant same-scope cannot be silently serialized while the cross-scope Total pair round-trips.
- Defect verified in source at four layers: writer collision check + keep-rule (xlsx-defined-names.ts, preserved.has(entry.name) / preserved.has(unescaped) — case-sensitive), route collision check (office-routes.ts line 3138-3140, new Set(preserveNames) + case-sensitive has), browser namePreserved (ExcelEditor.tsx, case-sensitive Set.has on both preserve sets), reader poisoned-name sweep (poisonedNames.has(modeled[i].name) — case-sensitive; a case-variant sibling of an unmodelable element would stay modeled and poison every names-dirty save).
- Implementation (commit 73158ce, 8 files): (1) WRITER applyDefinedNamesState — preserve set built lowercased; collision check compares entry.name.toLowerCase() (fails closed in every case combination); keep-rule compares unescaped.toLowerCase() so EVERY case-variant element of a preserved name is kept verbatim (a case mismatch can never silently drop an unmodeled element). (2) ROUTE expectDefinedNamesState — same lowercased-set check, 400 before bytes, every case combination. (3) BROWSER namePreserved — both preserve sets matched case-insensitively; create/rename refused up front in any case. (4) READER parseDefinedNamesState — poisonedNames stores lowercased names; the sweep strips modeled case-variant siblings of poisoned families (the whole case-variant family stays file-only, byte-preserved, uneditable; other names remain editable).
- Regressions added: gateway writer +3 (case-variant modeled/preserved collision fails closed in every combination Foo/foo|FOO/foo|foo/FOO|Foo/FOO; every case-variant element of a preserved name kept verbatim — no silent deletion; case-variant cross-scope pair 'Total'+'total' serializes as two distinct definitions), gateway reader +2 (case-variant family poisoned when one element is unmodelable; case-variant same-scope pair is one genuine duplicate — winner modeled, loser preserved), route +2 (case-variant collision rejected up front in every combination; Total workbook+sheet pair with empty preserve list accepted — valid round-trip shape reaching the engine stage), browser E2E +2 (18: case-variant same-scope duplicate fails the save closed with the writer's exact message + creating FOO at the Data-sheet scope refused up front with the preserve message; 19: case-variant cross-scope pair Total@workbook + total@Data installs both, editing the workbook one saves cleanly with the sheet-scoped twin byte-verbatim + _xlnm.Print_Titles + GlobalTotal intact, reopen reads both truths). Two new fixtures: buildExcelNamesCaseCollisionFixture, buildExcelNamesCasePairFixture.
- One mid-gate defect found and fixed: E2E 18's create-refusal assertion originally matched two elements (the dialog error + the lingering save-failure status toast share the phrase 'exists in a form the editor cannot model') — strict-mode violation; fixed by scoping the assertion to the name-manager-dialog.
- Gates on the corrected tree: typecheck ×3 green; gateway 706/706 (+5); contractor-core 479 passed + 4 skipped (+2); web 235/235 (architecture guards intact); FORMAT_BASE_REF=origin/main format:check clean (after prettier on the spec file); ESLint clean on all 8 changed files; production build green (19.9s); FULL browser E2E 141/141 (names 19/19 + full-suite regression 122 in 5 chunks); independent byte evidence (standalone script against the gateway source, outside the test files): 7/7 checks green — reader models winner + preserves case-variant loser; the exact state fails the save closed (DefinedNameError); every modeled/preserved case combination fails closed; the cross-scope Total pair models both and round-trips both; edit-one-preserves-other byte-verbatim. Frozen surfaces zero changes (diff confined to the PR #31 file set).
- Push + PR response: commit 73158ce pushed to PR #31; blocker-response comment posted (issuecomment 5449540026) mapping each of the architect's required corrections to its evidence.
- CI on 73158ce driven to conclusion: web GREEN (the canonical Excel gate); desktop-e2e green; Vercel Preview Comments success; test fails ONLY at Lint with 20 annotations file-for-file identical to the established pre-existing set (verified via the check-run annotations API; BOQ.tsx NOT in the diff — verified via git), zero in EXCEL-025 files; e2e fails ONLY at the frozen Electron shell step (job-steps API); foundation fails ONLY at Verify branch isolation (the structural cross-domain guard, identical on merged+verified PRs #23/#25/#29).
- Ledger updated in lockstep AFTER evidence: work-items.md EXCEL-025 second correction block + status line; verification-matrix.md second correction + CI line at head 73158ce + DEPLOYED updated.

Stage Summary:

- PR #31 second blocker fully addressed: the modeled∩preserve collision predicate is now case-insensitive at all four layers (writer collision check + keep-rule, route validation, browser up-front refusal, reader poisoned-name sweep); a case-variant same-scope duplicate (Foo + foo) fails the save closed in every case combination while the cross-scope pair (Total workbook + Total sheet, exact or case-variant) round-trips validly with the twin byte-verbatim. Workflow state: CHANGES ADDRESSED — PR #31 head 73158ce, web gate GREEN, awaiting the architect's re-review (ARCHITECT_REVIEW → APPROVED → MERGED → VERIFIED remain the architect's transitions). NOT VERIFIED.

---

Task ID: EXCEL-026-phase-a
Agent: Z.ai (Implementation Operator)
Task: EXCEL-026 (View / Page Layout persistence) Phase A forensic audit — read-only investigation of freeze/gridline/formula-view/page-setup state before any implementation, per architect handoff.

Work Log:

- Repository truth verified on a FRESH clone (environment was reset again): origin/main HEAD = 84ac05a "Merge EXCEL-025 Named Ranges"; PR #31 state=closed merged=true (merge commit 84ac05a = main); both prior blocker corrections (753f13a scope-aware grouping, 73158ce case-insensitive collision guard) are ancestors of main. Worktree clean; branch excel-026-view-persistence created from origin/main. EXCEL-024 = PR #29 remains the architect's queue item; not this task.
- (A) FREEZE — read: parseFrozenPane() (xlsx-gateway.ts) regex-parses <pane>, accepts state="frozen"|"frozenSplit", ySplit/xSplit → frozenRows/frozenColumns, undefined when 0/0; topLeftCell/activePane are NOT parsed (the canonical writer setFrozenPane regenerates them deterministically from the split counts — sufficient; proven by the existing 6 writer tests + the freeze round-trip E2E). Snapshot field WorksheetState.freeze (minimal, absent = no freeze). Browser import: ExcelEditor.tsx loadSnapshot → Univer freeze config {startRow: frozenRows|-1, startColumn, xSplit, ySplit}; journal seeded from snapshot. Journal: built-in sheet.mutation.set-frozen listener → freezeStateRef per sheet NAME. Save: pageSetupStates = freezeStateRef filtered to >0. Writer: applyPageSetupState → setFrozenPane (canonical pane; 0/0 REMOVES the pane and drops pane-scoped selections). DEFECT FOUND (clear direction): the browser listener DELETES the journal entry when freeze clears (frozenRows===0&&frozenColumns===0) and the save collection filters zeros out — a cleared freeze never reaches the save plan, so the file's <pane> element SURVIVES the save and the user's clear is silently lost on reopen. Desktop parity records {frozenRows: 0, frozenColumns: 0} as a REAL patch (App.tsx set-frozen listener → recordPageSetup), which the writer turns into pane removal. Route already accepts 0 (non-negative-integer bound). Fix planned: keep zeros in the journal (deletion only for "never had freeze"), emit them. Also: parseFrozenPane has NO direct gateway reader unit test today (writer-only freeze tests; round-trip proven only through E2E).
- (B) GRIDLINES — writer: applyPageSetupState showGridlines → setSheetViewAttr('showGridLines','0'|null) — canonical + tested. Reader: NOT parsed anywhere (parseWorksheetPresentation handles styles/merges/rows/cols/freeze only; WorksheetState has no showGridlines field) → read-side gap. Route: expectSheetPageSetupState validates ONLY frozenRows/frozenColumns (0 allowed; unknown keys ignored — documented forward seam); no size cap for pageSetupStates (filters=100, dv/cf=1000). Browser: toggleGridlines calls ws.setHiddenGridlines() — Univer 0.25.1 built-in FWorksheet facade → ToggleGridlinesCommand ('sheet.command.toggle-gridlines') → ToggleGridlinesMutation 'sheet.mutation.toggle-gridlines' params {unitId, subUnitId, showGridlines: 0|1} with engine-managed undo/redo (verified from installed @univerjs/sheets source). The web does NOT journal it today (ribbon tooltip says "in-session"); load hardcodes showGridlines: 1 for both the demo workbook and opened files. DESKTOP REFERENCE (persistence authority): sidecar reads sheetView@showGridLines → showGridLines (zod boolean) → Univer config showGridlines at createWorkbook (univer-sync.ts:361); View toggle → worksheet.setHiddenGridlines → App.tsx 'sheet.mutation.toggle-gridlines' listener → recordPageSetup({showGridlines}) → canonical pageSetupStates → gateway writes sheetView@showGridLines. Gridlines are genuinely PERSISTED workbook state on the desktop — the browser's "in-session only" is the gap.
- (C) FORMULA VIEW — OOXML: <sheetView showFormulas="1">, per-sheet view state persisted by Excel itself. Gateway: writer canonical + tested (setSheetViewAttr('showFormulas','1'|null)); reader does NOT parse it. DESKTOP: View → Show Formulas = per-sheet Set state.showFormulaSheets (SEEDED from the file via sidecar showFormulas on open, App.tsx:3193) + RENDER_RAW_FORMULA_KEY context flip (engine repaints skeletons; verified engine-render's subscription) + CELL_CONTENT interceptor priority 9999 swapping v→formula text (required because in 0.25.1 the skeleton's _renderRawFormula only reaches the rich-text/rotated cell path — verified from engine-render source line 18311: modelObject is only built with displayRawFormula when cellData.p || vertexAngle || centerAngle). Journaled via recordPageSetup({showFormulas}) → canonical writer. Conclusion: formula view IS persisted workbook state under the canonical architecture (desktop precedent + Excel's own sheetView semantics) — NOT UI-only state; it belongs in the pageSetupStates family. Web imports available with ZERO new dependencies: RENDER_RAW_FORMULA_KEY, SheetInterceptorService, INTERCEPTOR_POINT all re-exported by @univerjs/preset-sheets-core (typecheck-verified inside apps/web).
- (D) PAGE-SETUP FAMILY BOUNDARIES — SheetPageSetupState (gateway writer) is COMPLETE for: orientation, paperSize, scale, fitToWidth, fitToHeight, fitToPage, margins (normal/wide/narrow presets), printGridlines, printHeadings, showGridlines, showFormulas, showHeadings, printArea, printTitles, header/footer, rowBreaks, colBreaks, frozenRows/Columns. Desktop journals ALL of them through the same family (PageSetupJournalState + page-layout-actions.ts). Web emits ONLY frozenRows/Columns today; the web Page Layout tab carries five DISABLED placeholders (Margins, Orientation, Size, Width, Height) each naming the architectural reason (EXCEL-014 discipline). CLASSIFICATION for this increment per the frozen verification-matrix entry (freeze; gridline visibility; formula-view state; print orientation/margins/page setup supported by the canonical engine) + the handoff target classes: (1) freeze = regression + clear-defect fix; (2) gridlines = EXTEND CANONICALLY (read field + import + journal via the built-in mutation + save + reopen); (3) formula view = EXTEND CANONICALLY (read field + import + desktop-parity interceptor render + journal + save + reopen); (4) print orientation/margins/page setup = EXTEND CANONICALLY by wiring the five existing disabled placeholders (orientation portrait/landscape, margins 3 presets, paperSize 7 codes, scale 10-400, fit-width/height 0-5) — journal-only family, desktop reference semantics; (5) showHeadings = READ+IMPORT ONLY, browser TOGGLE DEFERRED: the desktop toggle mutates worksheet config and then performs render-manager viewport surgery to work around an engine viewport-shift bug (IRenderManagerService/SkeletonManager internals) — exactly the "legacy/internal machinery" the handoff says not to replicate; not in the frozen matrix list. DEFER/FAIL-CLOSED (not listed in the frozen matrix entry; no web UI placeholders exist): printArea, printTitles, header/footer, breaks, printGridlines/printHeadings toggles, zoom persistence (workbookView@zoomScale), theme (EXCEL-030).
- Malformed-value policy decided for the reader: xsd:boolean set {"1","true"} = explicit true, {"0","false"} = explicit false; any other value is ignored (treated as the schema default for BROWSER MODELING ONLY) — the raw attribute stays byte-preserved on no-op saves because such a sheet never enters pageSetupStates, and a user toggle overwrites with a definite canonical value (Excel's own repair behavior). Structurally ambiguous state is never silently rewritten by the browser: the browser only ever writes definite values it journaled from the engine's own mutation or a ribbon command.

Stage Summary:

- Phase A audit COMPLETE; no implementation code written. Capability matrix: writer family complete and tested; READ side is the gap (no showGridlines/showFormulas/showHeadings in WorksheetState); browser gaps = gridline journal missing (mutation exists in engine), formula-view absent (desktop interceptor pattern portable with zero new deps), freeze-clear silently dropped (defect), page-layout placeholders disabled; route validation covers only freeze + has no cap. Planned increment: gateway reader fields + tests → route validation + cap + tests → browser (journal generalization, freeze-clear fix, gridline journal, formula-view module, Page Layout wiring) → web arch guards → 10-scenario E2E per the handoff → full gates → PR. Branch excel-026-view-persistence @ 84ac05a.

---

Task ID: EXCEL-026-implementation
Agent: Z.ai (Implementation Operator)
Task: EXCEL-026 (View / Page Layout persistence) — implement the gridline/formula-view/freeze-clear/print-family surface through the canonical pageSetupStates family, run all local gates, push, open PR.

Work Log:

- Implementation (14 files): GATEWAY (xlsx-gateway.ts + workbook.types.ts): new WorksheetState.view field + parseSheetViewState() — exposes ONLY non-default <sheetView> display flags (showGridlines:false / showFormulas:true / showHeadings:false; xsd "1"/"true"/"0"/"false" literals; malformed values ignored for modeling so the raw attribute stays byte-preserved on no-op saves — a sheet with unmodeled view state never enters pageSetupStates, and a user toggle replaces the malformed value with a definite canonical one). ROUTE (office-routes.ts): expectSheetPageSetupState extended to strictly validate showGridlines/showFormulas/showHeadings (booleans), orientation ('portrait'|'landscape'), margins (3 presets), paperSize (1..118), scale (10..400), fitToWidth/fitToHeight (0..1000), fitToPage (boolean) — every wired field validated BEFORE bytes — plus MAX_PAGE_SETUP_STATES=100 with the save-route cap check (the family previously had NO cap). BROWSER: (a) the per-sheet journal generalized to PageSetupJournalEntry (desktop PageSetupJournalState shape for every wired field) keyed by sheet name in pageSetupRef; (b) FREEZE-CLEAR DEFECT FIX — the old listener DELETED the journal entry on a 0/0 clear and the save collection filtered zeros, so a cleared freeze never reached the save plan and the file silently kept its <pane>; the clear now journals {frozenRows:0, frozenColumns:0} (desktop recordPageSetup parity) and the writer removes the pane; (c) GRIDLINE JOURNAL — sheet.mutation.toggle-gridlines (the engine's own mutation from FWorksheet.setHiddenGridlines) journals {showGridlines: boolean}; file-backed workbooks only (computed hasFile probe — desktop parity); (d) FORMULA VIEW — new apps/web/src/office/formula-view.ts (desktop formula-view.ts parity): per-sheet Set + RENDER_RAW_FORMULA_KEY context flip + CELL_CONTENT interceptor priority 9999 (the engine's _renderRawFormula only reaches rich-text/rotated cells in 0.25.1 — verified from engine-render source); interceptor installed once per runtime, ActiveSheetChanged re-syncs the global key to the active sheet's flag, seeded from the file on open; View → Show Formulas button added; (e) PAGE LAYOUT TAB — the five former disabled placeholders became live selects (Orientation, Margins, Size=7 OOXML paper codes, Scale to Fit Width/Height + Scale) with the desktop's exact semantics (scale disengages fitToPage; fit derives fitToPage from either axis; untouched axis keeps its journaled value or Automatic); (f) IMPORT — sheet.view seeds the live Univer config (showGridlines, rowHeader/columnHeader.hidden for showHeadings — desktop univer-sync parity) and the formula-view set; (g) SAVE — every journaled sheet emits {sheetName, ...entry}; sheets with no entry keep their view/page XML byte-for-byte.
- Scope discipline (per the frozen verification matrix + handoff): showHeadings = READ+IMPORT ONLY (the desktop toggle requires render-manager viewport surgery around an engine viewport-shift bug — exactly the "legacy/internal machinery" the handoff says not to replicate); printArea/printTitles/header-footer/breaks/printGridlines = OUT OF SCOPE (not in the matrix entry; no web placeholders existed; unknown keys still ride the route's documented forward seam).
- Tests added: gateway xlsx-view-state.test.ts 25/25 (reader existing/absent/explicit-default/xsd-literals/malformed/malformed-sibling/missing-sheetViews; parseFrozenPane unit gap: frozen/split-rejected/frozenSplit/zero-splits; write→reopen, edit→save→reopen, clear/reset both flags + pane removal, malformed→canonical replacement, freeze set→clear round-trip; print family through the applyCellEditsToXlsx slot incl. an untouched blackAndWhite attribute verbatim; no-op byte preservation via touchedEntries+sha256; per-sheet isolation; unknown-sheet fail-closed — two of my initial expectations were WRONG and corrected against canonical behavior: fitToWidth=1 is the OOXML default so the writer correctly drops it, and an unknown sheet name correctly FAILS CLOSED rather than being ignored). Route office-page-setup-routes.test.ts 20/20 (freeze regression incl. 0/0 clear + OOXML bounds; boolean view flags; print enums/ranges; unknown-key seam with a wired invalid field still 400ing; cap 100/101; non-array). Web arch guards +6 (no view/page OOXML/JSZip; no second persistence model; type-only wire; single journal family incl. the 0/0 clear + toggle-gridlines + formula-view module shape; ribbon wiring with no disabled stubs; formula-view module imports). Browser E2E ribbon-view-persistence.spec.ts 6 tests / 11 scenarios covering every scenario the handoff mandates (freeze import/edit/clear save-reopen; gridline import/toggle/toggle-back; formula-view render proof (pixel-level: toggle-off CHANGES the grid render, toggle-back returns the byte-exact imported render) + persistence; unrelated cell edits preserve all three view truths + <sheetViews> byte-verbatim + untouched sibling sheet byte-identical + default-view file gains no view attributes; Page Layout print family round-trip with exact XML assertions). New fixture builder buildExcelViewFixture.
- Gates on the final tree: typecheck ×3 clean (gateway, contractor-core, web — one E2E-spec type error found and fixed); gateway 731/731; contractor-core 499 passed + 4 skipped; web 241/241; ESLint clean on all 14 files (one prefer-const auto-fixed); FORMAT_BASE_REF=origin/main format:check clean (after prettier on 6 files); production build green (19.14s); FULL browser E2E 147/147 in 7 batches (28+32+22+19+21+9+16; the background single-run approach died with the sandbox's process-group cleanup between tool calls — the batch pattern from prior sessions is the reliable path). Frozen surfaces: ZERO changes.
- Worklog + spec ledger updates: work-items.md EXCEL-026 entry rewritten with the Phase A audit summary, implementation map, scope classification, and full coverage inventory; verification-matrix.md EXCEL-026 evidence block added (UNIT/ARCH/BROWSER/frozen-surfaces/DEPLOYED-pending).
...
Task ID: EXCEL-027-phase-a
Agent: Z.ai (Implementation Operator)
Task: EXCEL-027 (Advanced cell formatting) Phase A forensic audit — read-only investigation of the canonical style pipeline before any implementation, per architect handoff.

Work Log:

- Repository truth verified on a FRESH clone (environment was reset again — PAT re-persisted): origin/main HEAD = db4414d "Merge EXCEL-026 View / Page Layout persistence" (PR #35 merged; CI web gate green at head 67500a7 per worklog + verification-matrix). Worktree clean; branch excel-027-advanced-formatting created from origin/main. Spec files read in full (architecture-lock, requirements, dependency-graph, verification-matrix, work-items). Authoritative EXCEL-027 requirement list extracted from the frozen verification matrix: borders and line styles; text rotation; indentation; built-in and custom number formats; named cell styles where supported; paste special; format painter; save/reopen; XML style assertions; browser + production E2E.
- (1) CANONICAL STYLE READER (gateway StylesheetReader, xlsx-styles.ts): resolves bold/italic/underline/strike/fontSize/fontFamily/fontColor/fillColor/numberFormat (custom ≥164 via <numFmts> + built-ins)/hAlign/vAlign/wrapText → CellFormatState. CellFormatState (workbook.types.ts:38-40) DECLARES textRotation/indent/border fields but resolve() never populates them. Borders section is not even parsed by the reader (only fonts/fills/cellXfs/numFmts/dxfs). Diagonal borders: no model field. Vertical/stacked text: type allows 'vertical', never set. XF/style-ID preservation: INTACT — the copy-on-write StylesheetEditor + byte-preserving save path keep untouched cells' cellXfs/borders byte-for-byte (proven by existing dedupe/preservation tests). READ-SIDE GAPS: borders (per-edge), textRotation, indent.
- (2) CANONICAL STYLE WRITER (StylesheetEditor + WorkbookStyleEdit): WorkbookStyleEdit ALREADY carries borderTop/Bottom/Left/Right {style: all 13 OOXML ST_BorderStyle names, color?} | null (null clears the edge), textRotation 0..255 (0 clears the attribute), indent (0 clears) — serializable TODAY with zero engine changes. buildBorder rebuilds only delta edges in schema order; untouched edges + <diagonal> + border attrs (diagonalUp/Down) preserved verbatim; null → bare <edge/>. buildAlignment handles rotation/indent deltas and carries unmodeled alignment attrs (relativeIndent/justifyLastLine/shrinkToFit/readingOrder). Route expectStyleEdit ALREADY validates all of it (13-style enum, #RRGGBB colors, rotation 0..255, indent 0..15) BEFORE bytes. Gateway test coverage today: rotation/indent write = 3 tests (xlsx-save-edits); border deltas = ZERO tests. CLASSIFICATION: borders(4 sides/13 styles/color/clear/side-isolation) = EXTEND CANONICALLY (reader+journal+UI; writer/route SUPPORTED NOW); textRotation = EXTEND CANONICALLY (same); indentation = EXTEND CANONICALLY (same); number formats built-in+custom = SUPPORTED NOW both sides EXCEPT the browser never seeds CellFormatState.numberFormat into the live model on import (formatToUniverStyle drops it) → small EXTEND for import rendering; named cell styles = FAIL CLOSED/DEFER (no canonical engine path; <cellStyles> stays byte-preserved; "where supported" resolves to NOT supported); paste special = DEFER (no canonical path; clipboard semantics are not a style-delta family); format painter = DEFER (same); diagonal borders = FAIL CLOSED/DEFER (writer preserves verbatim; no delta model; desktop journals only t/b/l/r).
- (3) DESKTOP REFERENCE (read-only): Home → Borders dropdown = 8 presets (all/outer/thick-outer/top/bottom/left/right/none) + separate border-color dropdown, default style thin (thick-outer→MEDIUM); Format Cells dialog Border tab adds a 7-name line-style picker (thin/medium/thick/double/hair/dashed/dotted) + color. Command `border:<preset>:<color>[:<style>]` → FRange.setBorder(type, style, color). Home → Orientation dropdown = 6 presets (45/-45/vertical/90/-90/0=clear); Format Cells Alignment tab mirrors. Command `rotate:<value>` → 'sheet.command.set-text-rotation' {value}. Indent = Format Cells dialog only (`indent:<steps>` 0..250 → pd padding). Import: sidecar WorkbookCellStyle (borders incl. diagonal + rotation + indent + numberFormat) → toUniverStyle() seeds Univer bd{t,b,l,r,tl_br,bl_tr}/tr/pd/n. Journal: toNeutralStyle() maps Univer bd/tr/pd → WorkbookStyleEdit border edges/textRotation/indent with the OOXML angle convention (univer a counterclockwise-positive -90..90 → ooxml 0-90 up / 91-180 down (90-a) / 255 stacked; null=clear→0); fromNeutralStyle() is the exact inverse; BORDER_EDGE_KEYS = t/b/l/r only. Selection echo: SelectionFormat carries textRotation + indent; borders have NO echo by design ("presets cannot represent arbitrary per-edge state"). Save: journal CellEdit.style → same gateway wire.
- (4) UNIVER 0.25.1 AUDIT (installed source, all PUBLIC): FRange.setBorder(type: BorderType, style: BorderStyleTypes, color?) — BorderType includes TOP/BOTTOM/LEFT/RIGHT/NONE/ALL/OUTSIDE/INSIDE/HORIZONTAL/VERTICAL (+diagonals); BorderStyleTypes 1..13 = the 13 OOXML styles 1:1. SetBorderBasicCommand ('sheet.command.set-border-basic') → SetBorderCommand → executes SetRangeValuesMutation ('sheet.mutation.set-range-values') with the bd cellValue payload; engine-managed undo/redo. SetTextRotationCommand ('sheet.command.set-text-rotation') {value: number | non-number→vertical} → SetStyleCommand → set-range-values with s.tr; engine-managed undo/redo. IStyleData: bd {t,b,l,r} each {s: BorderStyleTypes, cl: {rgb}}, tr {a} | {v:1}, pd {l:px}. CRITICAL: BOTH advanced-formatting entry points land on the SAME 'sheet.mutation.set-range-values' mutation family the web journal ALREADY subscribes to — the existing style journal captures them with ZERO new listeners once styleDeltaFromUniver maps tr/pd/bd (today it maps bl/it/ul/st/ff/fs/cl/bg/ht/vt/tb only — THE GAP). Read-back: FRange.getCellStyleData() returns the resolved IStyleData including bd/tr/pd — public. Univer rotation semantics: a = counterclockwise-positive -90..90, 'v' = stacked; NARROWER than raw OOXML only in that it can't express >180 or non-integer — the canonical subset (integers -90..90 + vertical) is exactly what Excel's UI offers.
- (5) EXISTING WEB UI: Home ribbon has undo/redo, font family/size, B/I/U, font/fill color, h/v align, wrap, merge, Number-format select, CF — NO border picker, NO orientation/rotation, NO indent. Smallest UI matching desktop behavior + frozen matrix: Home → Font/Alignment area gains a Borders preset select (desktop's 8 presets + line-style select + border-color picker) and an Orientation select (desktop's 6 presets). Indent: desktop exposes it ONLY through the Format Cells dialog (no Home buttons) — the web has no Format Cells dialog (EXCEL-032), so a single indent control would EXCEED the desktop surface; classify indentation UI as DEFERRED to EXCEL-032 Dialog Parity, while the reader+journal+wire side is implemented now (state survives round-trip; UI lands with the dialog). formatToUniverStyle import seeding extended with bd/tr/pd/n so imported advanced formatting RENDERS (desktop toUniverStyle parity).

Stage Summary:

- Phase A audit COMPLETE; no implementation code written. The canonical style family ALREADY owns borders/rotation/indent end-to-end on the write side (WorkbookStyleEdit + StylesheetEditor + route validation) — the work is the READ side (StylesheetReader per-edge borders/textRotation/indent), the BROWSER journal mapping (styleDeltaFromUniver tr/pd/bd), import RENDER seeding (formatToUniverStyle + n pattern), and the Home ribbon Borders + Orientation controls (public FRange.setBorder / set-text-rotation → same set-range-values mutation family → existing save pipeline). No new persistence family; no browser OOXML; desktop parity followed through the neutral-style conversion conventions. Scope: borders + line styles + color + side isolation + clear; text rotation (positive/negative/vertical/clear); number-format import rendering; indent read/journal/wire (UI deferred to EXCEL-032); named cell styles / paste special / format painter / diagonals = documented fail-closed/deferred. Next: implementation per gate order.

---

Task ID: EXCEL-027-implementation
Agent: Z.ai (Implementation Operator)
Task: EXCEL-027 (Advanced cell formatting) — implement borders/line styles/colors/text rotation (+indent wire side, +number-format import rendering) through the ONE canonical WorkbookStyleEdit family, run all local gates, push, open PR.

Work Log:

- Implementation (16 files: 13 modified + 3 new). GATEWAY: (a) domain/workbook.types.ts — CellFormatState.border re-typed from the demo-DSL BorderPatch to the per-edge CellBorderState {top?,bottom?,left?,right?} each {style: CellBorderStyleName (the 13 ST_BorderStyle values), color?} (colors follow the established read-side convention: NO '#'); textRotation documented as the OOXML convention (1..90 ccw, 91..180 cw, 'vertical' = 255); (b) xlsx-styles.ts StylesheetReader — parses the borders section (constructor) and resolves per-edge border state + textRotation + indent in resolve(): edges appear only when the file carries one of the 13 canonical style names (unknown styles like "wave" are NOT modeled — fail closed, XML preserved); rotation 255 → 'vertical', 181..254/malformed ignored; indent ≥1 only; (c) index.ts exports the new types. WRITER: ZERO changes (border/rotation/indent deltas were already canonical). ROUTE (office-routes.ts): (a) DEFECT FIX — expectStyleEdit DROPPED border null-clears (`borderTop: null` was silently discarded by the `!== null` guard, so a cleared border never reached the engine and the file's border survived the save — the same defect class as EXCEL-026's freeze-clear); new expectStyleBorderEdge passes null through as a REAL clear; (b) indent bound 0..15 → 0..250 (aligned with the OOXML CT_CellAlignment domain, the canonical writer, and the desktop's dialog/journal which both accept 0..250) + integer enforcement for indent AND textRotation (previously non-integers could reach the writer and produce schema-invalid attributes). BROWSER: (a) cell-mutation-merge.ts styleDeltaFromUniver extended with the desktop toNeutralStyle parity mappings: bd (t/b/l/r → borderTop/Bottom/Left/Right; null edge → null clear — exactly what the engine's clearBorder emits; diagonal keys ignored; unknown style numbers skipped) + tr (null → 0; {v:1} → 255; {a} clamped -90..90 → OOXML a>=0 ? a : 90-a) + pd (null → 0; {l px} → round(l/INDENT_STEP_PX) clamped 0..250); shared conversion tables exported (UNIVER_BORDER_TO_XLSX 1..13, XLSX_BORDER_TO_UNIVER, INDENT_STEP_PX=8, ooxmlTextRotationToUniver) for reuse by the import seeding; (b) ExcelEditor.tsx formatToUniverStyle — import render seeding (desktop toUniverStyle parity): n {pattern} (number formats now RENDER on import, not just persist), bd per-edge {s, cl {rgb '#'-normalized}}, tr (OOXML → {a}/{v:1}), pd {l: indent*8}; (c) useExcelRuntime.ts — applyBorderPreset (PUBLIC FRange.setBorder(type, lineStyle, rgb); preset→BorderType mapping = desktop BORDER_COMMAND_TYPES parity incl. thick-outer→OUTSIDE+MEDIUM) + setOrientation (PUBLIC 'sheet.command.set-text-rotation' {value} — the desktop's exact call shape; 'v' for vertical); (d) Ribbon.tsx Home tab — Borders preset select (8 desktop presets) + border line-style select (7 desktop dialog styles) + border-color picker in the Font group + Orientation select (6 desktop presets) in the Alignment group; BorderColorIcon added. CRITICAL ARCHITECTURAL FACT (from the Phase A Univer audit): BOTH new entry points land on the SINGLE 'sheet.mutation.set-range-values' mutation family (SetBorderBasicCommand → SetBorderCommand → SetRangeValuesMutation; set-text-rotation → SetStyleCommand → SetRangeValuesMutation) with engine-managed undo/redo — the existing journal subscription captures them with ZERO new listeners; no second persistence family anywhere.
- Tests: gateway xlsx-advanced-formatting.test.ts 23/23 (new two-sheet fixture with mixed per-side styles/colors, diagonal+attrs border, rotations 45/135/255, indent 2, the COMBINED cell bold+fill+numFmt+align+border+rotation, an unknown "wave" style, and the out-of-domain rotation 200; reader: per-edge resolution, diagonal skip, unknown-style/out-of-domain fail-closed, empty default; writer: all 13 line styles round-trip, color round-trip, one-side edit preserving other sides + diagonal + diagonalUp/Down verbatim, null-edge clear, all-side clear, combined-fixture preservation under border/rotation/bold edits, rotation write/clear/vertical with the cell's own xf attribute check, indent write/clear, style-only edits never touch values, schema-order serialization, border interning; untouched sibling sheet byte-preservation). Route office-style-routes.test.ts 11/11 (all 13 styles × 4 sides accepted, null clears pass through, non-object edges/unknown keywords/malformed colors rejected, rotation 0..255 integer, indent 0..250, combined payload, unknown-key seam). Web unit +9 (bd/tr/pd mapping incl. the engine clearBorder all-null form, {a:0}→0, {a:-45}→135, diagonal+unknown fail-closed, indent clamp, combined families, merge with prior value edit; ooxmlTextRotationToUniver table). Architecture guards +7 (no border/rotation OOXML or JSZip in apps/web/src; no borderStates/rotationStates/advancedFormatStates; journal through the existing style family with shared conversion tables; public-API-only entry points — no as unknown as / _injector in the new callbacks; import seeding parity; ribbon wiring with no disabled stubs; snapshot-field read-back). Browser E2E ribbon-advanced-formatting.spec.ts 7 tests / ALL 14 mandated scenarios (import + live-model read-back through the PUBLIC FRange.getCellStyleData + pixel-level render proofs — clearing the imported border/rotation changes the rendered pixels; one-side edit with side isolation at journal/XML/reopen layers incl. the engine's adjacent-edge clear semantics and the diagonal-preserving border entry; No Border clear as four real nulls; rotation 45/-45/vertical + 0-clear round-trips with exact textRotation XML assertions; the combined-format guard; no-op preservation — value-only save leaves styles.xml and the sibling sheet byte-identical with the wave border intact). New E2E fixture buildExcelAdvancedFormatFixture.
- Two of my initial E2E expectations were WRONG and corrected against canonical engine behavior: (1) the engine models a cleared rotation as {a: 0} (or drops tr) — never null — the JOURNAL maps it to the 0 clear sentinel (unit-tested); (2) the No Border preset ALSO clears the adjacent outside cells' shared edges (the engine's own outline-border semantics — outlineBorder emits b:null above, t:null below, r:null left-of, l:null right-of), so B1's shared left edge is cleared and its new border entry preserves the diagonal + diagonalUp/Down attrs — faithful journaling of the engine's own mutation, desktop parity.
- Gates on the final tree: typecheck ×3 clean (gateway, contractor-core, web); gateway 754/754; contractor-core 510 passed + 4 skipped; web 257/257; FORMAT_BASE_REF=origin/main format:check clean (after prettier on 9 files); ESLint clean on all 16 changed files; production build green (18.63s); FULL browser E2E **154/154** in 9 batches (18+11+21+22+10+41+15+9+7 — the 147 established regression suite + the 7 new tests; the style pipeline touched by EXCEL-027 is exercised by excel-format, ribbon-home-persistence, and every formula/sort/filter/DV/CF/notes/protection/table/image/chart/named-ranges/view spec — all green). Frozen surfaces: ZERO changes. Independent byte evidence captured (scripts/verify-027-byte.mts): read → one-side border write (right thick #123456 added; top thin C00000 + bottom medium 0000FF preserved verbatim in ONE new border entry) + rotation 135 write → reopen (3-sided border + bold/fill/rotation 45 all intact) with the untouched sibling sheet byte-identical.
- Ledger updates: work-items.md EXCEL-027 entry (Phase A summary, scope classification, defect fixes, full coverage inventory, status IMPLEMENTED); verification-matrix.md EXCEL-027 evidence block (UNIT/ARCH/BROWSER/frozen-surfaces/CI-pending/DEPLOYED-pending).
