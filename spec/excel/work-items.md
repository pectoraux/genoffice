...
Status: CHANGES REQUESTED by architect on PR #31 (2026-08-28) — TWO blockers raised and corrected: (1) scope-collapsing reader, (2) case-sensitive modeled∩preserve collision checks; both corrections implemented and pushed (see the correction evidence blocks below), awaiting architect re-review. NOT MERGED, NOT VERIFIED — the architect owns ARCHITECT_REVIEW → APPROVED → MERGED → VERIFIED.
...
Architect blocker correction evidence (2026-08-28, PR #31 — CHANGES REQUIRED → correction pushed):

- Blocker (pectoraux, PR #31 conversation): `parseDefinedNamesState()` grouped defined names by `entry.name.toLowerCase()` ALONE, collapsing legitimate workbook-scoped + sheet-scoped definitions sharing a name; the canonical writer keys uniqueness on `(name, sheetIndex)`, and the collapsed loser became a `preserveNames` entry colliding with the modeled winner — silently losing a valid sheet-scoped definition on open and failing subsequent saves incorrectly.
- Correction: (1) reader grouping key is now `(case-insensitive name, sheetIndex ?? -1)` — the writer's exact uniqueness key — so a same-name cross-scope pair models BOTH definitions; only GENUINE same-scope duplicates (which Excel never writes) collapse (live over #REF!, winner modeled, losers preserved — the architect-endorsed fail-closed). (2) Writer + route duplicate keys made case-insensitive to match (Excel resolves names case-insensitively). (3) New reader rule: a name preserved for an UNMODELABLE element (dangling scope/empty body/unsaveable name) never carries a modeled entry at any scope (preserveNames is name-granular — modeling one element of such a name would make every names-dirty save fail the writer's collision guard); duplicate-group losers deliberately keep the architect-endorsed modeled-winner + preserved-loser combination. (4) Browser install: the engine's builder validation is name-keyed and scope-blind (verified from installed @univerjs/sheets 0.25.1 source — `build()` → `validateDefinedName` → `getValueByName`), while the underlying DefinedNamesService is id-keyed (holds/lists/edits/deletes/structurally-shifts same-name siblings per id, verified: `UpdateDefinedNameController` iterates the full id-keyed map); each case-insensitive name group installs its first entry through the validated builder and the same-name siblings through the same PUBLIC `insertDefinedNameBuilder` facade with a hand-filled public `ISetDefinedNameMutationParam` (no private internals, no second persistence model). (5) Name Manager create/rename enforce Excel's scope rule (same name across scopes legal, within one scope refused case-insensitively; names in either preserve set refused). (6) Name Box resolution uses Excel scope precedence: the definition scoped to the ACTIVE sheet shadows the workbook-level one; workbook applies elsewhere; other-sheet scoped names remain jump targets (desktop parity).
- New coverage: gateway reader +8 tests (same-name workbook+sheet scope modeled as distinct definitions; case-variant across scopes; the 3-scope Excel_Version all modeled; genuine same-scope duplicate winner+loser; poisoned-name never modeled; pair write→reopen round-trip; edit-one-preserves-twin byte-verbatim; delete-one-preserves-other both directions) + writer +1 (case-insensitive same-scope duplicate rejected; case-variant cross-scope legal) + route +2 (case-insensitive same-scope duplicate rejected; case-variant cross-scope accepted). Browser E2E redesigned 11 (genuine same-scope duplicate fails the save closed) + 6 new scenarios (12: pair opens/installs/lists both with true scopes; 13: Name Box resolves the pair with Excel scope precedence incl. case-insensitivity and cross-sheet resolution; 14: editing one preserves the other BOTH directions with save/reopen + byte-verbatim twin; 15: deleting one preserves the other; 16: a structural edit shifts BOTH definitions through the split-save; 17: creating a same-name pair at a new scope works while the same scope is refused with the scope-aware message). Suites after correction: gateway 701/701, contractor-core 477+4sk, web 235/235, full browser E2E 139/139, frozen surfaces zero changes. Known engine limitation (documented in the PR): Univer 0.25.1's formula-engine name resolution is name-keyed and scope-blind — cell formulas referencing an AMBIGUOUS same-name resolve to the last-registered entry; the FILE preserves both scopes correctly (canonical bytes are the contract), and Name Box/Name Manager are scope-correct in the browser.

Second architect blocker correction evidence (2026-08-28, PR #31 — CHANGES STILL REQUIRED → correction pushed, head 73158ce):

- Blocker (pectoraux, PR #31 review, 2026-08-28T06:00:50Z): the FIRST correction's reader groups case-variant duplicates case-insensitively (a same-scope `Foo` + `foo` pair → winner modeled, loser preserved), but `applyDefinedNamesState()` and the route's `expectDefinedNamesState()` still compared `preserveNames` CASE-SENSITIVELY (`preserved.has(entry.name)`) — a names-dirty save could emit BOTH the modeled `Foo` and the preserved `foo` instead of failing closed.
- Correction: (1) WRITER `applyDefinedNamesState`: the preserve set is built lowercased and the modeled∩preserve collision check compares `entry.name.toLowerCase()` — the collision fails closed in every case combination; the keep-rule is case-insensitive too, so EVERY case-variant element of a preserved name is kept verbatim (a case mismatch can never silently drop an unmodeled element). (2) ROUTE `expectDefinedNamesState`: the same lowercased-set check — the collision surfaces as a 400 BEFORE the engine touches bytes, in every case combination. (3) BROWSER `namePreserved`: Name Manager create/rename matches both preserve sets case-insensitively — creating `FOO` when `foo` is preserved is refused up front (no round-trip to the 400). (4) READER `parseDefinedNamesState`: the poisoned-name sweep is case-insensitive — a case-variant sibling of an unmodelable element (dangling scope/empty body/unsaveable name) stays file-only rather than being modeled and poisoning every names-dirty save on a guaranteed collision.
- New coverage: gateway writer +3 (case-variant modeled/preserved collision fails closed in every combination; every case-variant element of a preserved name kept verbatim — no silent deletion; case-variant cross-scope pair serializes as two distinct definitions) + reader +2 (case-variant family poisoned when one element is unmodelable; a case-variant same-scope pair is one genuine duplicate — winner modeled, loser preserved) + route +2 (case-variant collision rejected up front in every combination; the Total workbook+sheet pair accepted with an empty preserve list — valid round-trip shape). Browser E2E +2: 18 (a case-variant same-scope duplicate Foo+foo fails the save closed with the writer's exact message; creating FOO at the Data-sheet scope refused up front with the preserve message) and 19 (a case-variant cross-scope pair Total+total installs both, editing the workbook one saves cleanly with the sheet-scoped twin byte-verbatim + print titles + unrelated names intact, reopen reads both truths). Independent byte evidence (standalone script against the gateway source, outside the test files): 7/7 checks green. Suites after correction: gateway 706/706, contractor-core 479+4sk, web 235/235, full browser E2E 141/141 (names 19/19), frozen surfaces zero changes. CI on 73158ce: `web` gate GREEN; `test` fails only at Lint with the 20 pre-existing annotations (file-for-file identical, zero in EXCEL-025 files, BOQ.tsx not in the diff); `e2e` fails only at the frozen Electron shell step; `foundation` fails only at the structural cross-domain isolation guard; `desktop-e2e` green. Blocker response posted on PR #31 (issuecomment 5449540026) mapping each required correction to its evidence. Status: CHANGES ADDRESSED — awaiting the architect's re-review. NOT MERGED, NOT VERIFIED.
...
Dependencies: EXCEL-015.
Status: implemented (PR pending) — gridline visibility, formula-view state, freeze-clear defect fix, and the print page-setup family all persist through the canonical `pageSetupStates` family.

Phase A forensic audit (2026-08-28, read-only, recorded in the worklog): the canonical writer (`applyPageSetupState`) already supported the entire view/page family (sheetView@showGridLines / @showFormulas / @showRowColHeaders, `<pane>`, orientation/paperSize/scale/fit/margins); the read side exposed only freeze, the browser wired only freeze, and the route validated only freeze with no cap. Desktop reference verified: gridlines journal through the engine's built-in `sheet.mutation.toggle-gridlines`; formula view is genuinely persisted per-sheet state (sidecar read → per-sheet set → RENDER_RAW_FORMULA_KEY + CELL_CONTENT interceptor → recordPageSetup) — NOT UI-only state. DEFECT FOUND in the freeze foundation: the browser's journal DELETED the entry on a freeze clear and the save collection filtered zeros, so a cleared freeze never reached the save plan and the file silently kept its `<pane>`.

Implementation (branch `excel-026-view-persistence`):

- Gateway reader: `WorksheetState.view` — non-default `<sheetView>` display flags only (`showGridlines: false`, `showFormulas: true`, `showHeadings: false`; xsd:boolean literals accepted; malformed values ignored for modeling — the raw attribute stays byte-preserved because such a sheet never enters pageSetupStates, and a user toggle replaces it with a definite canonical value). parseFrozenPane unit-gap filled (frozen/split/frozenSplit/zero-split coverage).
- Route validation: `expectSheetPageSetupState` now strictly validates showGridlines/showFormulas/showHeadings (booleans), orientation/margins (enums), paperSize (1..118), scale (10..400), fitToWidth/fitToHeight (0..1000), fitToPage (boolean) — every wired field validated BEFORE the engine touches bytes — plus MAX_PAGE_SETUP_STATES=100; unknown keys still ride the documented forward seam.
- Browser: freeze/gridlines journal through the ONE generalized `pageSetupRef` (desktop PageSetupJournalState shape); the freeze CLEAR now journals {0,0} (writer removes the pane — defect fix); gridlines journal from the engine's own `sheet.mutation.toggle-gridlines` (file-backed workbooks only — desktop parity); formula view renders through the desktop-parity `office/formula-view.ts` (per-sheet set + RENDER_RAW_FORMULA_KEY + CELL_CONTENT interceptor, zero new dependencies) and journals {showFormulas}; View → Show gained Show Formulas (Gridlines tooltip updated to "persists on save"); the Page Layout tab's five former disabled placeholders became live selects (Orientation, Margins, Size, Scale to Fit Width/Height + Scale) with the desktop's exact scale/fit semantics (scale disengages fitToPage; fit derives fitToPage from either axis); import seeds the live config from `sheet.view` (gridlines, hidden headings) and the formula-view set.
- Scope classification per the frozen verification matrix: freeze = regression + defect fix; gridlines + formula view + print orientation/margins/page setup = the matrix's required surface; showHeadings = read/import ONLY (browser toggle deferred — the desktop toggle path requires render-manager viewport surgery around an engine bug, exactly the "legacy/internal machinery" the handoff says not to replicate); printArea/printTitles/header-footer/breaks/print-gridlines = out of scope (not in the matrix entry; no web placeholders existed).

Coverage: gateway `xlsx-view-state.test.ts` 25/25 (reader: existing/absent/explicit-default/xsd-literals/malformed/malformed-sibling/missing-sheetViews; freeze reader unit-gap; write→reopen, edit→save→reopen, clear/reset both flags + pane removal, malformed→canonical replacement, freeze set→clear round-trip; print family through the applyCellEditsToXlsx slot incl. verbatim untouched attributes; no-op byte preservation; per-sheet isolation; unknown-sheet fail-closed) + route `office-page-setup-routes.test.ts` 20/20 (freeze regression incl. 0/0 clear + bounds; boolean view flags; print enums/ranges; unknown-key seam; cap 100 accepted/101 rejected; non-array) + web architecture guards +6 (no view/page OOXML or JSZip in apps/web/src; no second persistence model; type-only wire; single journal family incl. the 0/0 clear + toggle-gridlines wiring + formula-view module shape; ribbon wiring with no disabled stubs) + browser E2E `ribbon-view-persistence.spec.ts` 6 tests / 11 scenarios (freeze import, freeze edit save/reopen, freeze CLEAR save/reopen — the defect fix proven end-to-end, gridline import + per-sheet isolation, gridline toggle persists, toggle-back restores the default (attribute dropped), formula-view import + pixel-level render proof (toggle off changes the grid render, toggle back returns the EXACT imported render) + save/reopen, unrelated cell edit preserves all three view truths + `<sheetViews>` byte-verbatim through a non-view save + untouched sibling sheet byte-identical + default-view file gains no view attributes, Page Layout print family round-trip with exact XML attribute assertions).
...
Status: IMPLEMENTED (branch `excel-027-advanced-formatting`) — awaiting architect review.

Phase A audit (recorded in worklog, 2026-08-28): the canonical style family ALREADY owned borders/rotation/indent on the write side (`WorkbookStyleEdit` border edges with all 13 ST_BorderStyle names + hex colors + null-clears, `textRotation` 0..255, `indent`; the copy-on-write `StylesheetEditor` with per-edge side isolation and verbatim diagonal preservation; route `expectStyleEdit` validating all of it). The gaps were the READ side (StylesheetReader resolved no borders/rotation/indent), the browser journal mapping (styleDeltaFromUniver ignored `bd`/`tr`/`pd`), import render seeding (formatToUniverStyle dropped borders/rotation/indent/numberFormat), and the missing Home-ribbon UI. Univer 0.25.1 audit (installed source): BOTH advanced entry points — `FRange.setBorder(type, style, color)` (public facade, BorderType incl. ALL/OUTSIDE/INSIDE + per-side, BorderStyleTypes 1..13 = the 13 OOXML styles) and `'sheet.command.set-text-rotation'` — land on the SINGLE `sheet.mutation.set-range-values` mutation family with engine-managed undo/redo, so the existing web journal captures them with zero new listeners. Desktop reference (read-only): Home Borders dropdown (8 presets) + border color + Format Cells line-style picker (7 names); Orientation dropdown (45/-45/vertical/90/-90/0); `toNeutralStyle`/`fromNeutralStyle`/`ooxmlTextRotationToUniver` conversion conventions (OOXML 1..90 up / 91..180 down / 255 stacked ↔ Univer ccw-positive `a` / `{v:1}`).

Scope classification per the frozen verification-matrix entry: borders + line styles + colors + side isolation + clears = EXTEND CANONICALLY (implemented: reader + journal + import seeding + Home Borders preset/line-style/color controls through the public facade); text rotation (positive/negative/vertical/clear) = EXTEND CANONICALLY (implemented, same layers, Orientation dropdown); indentation = reader + journal + wire implemented (indent 0..250 aligned across route/writer/desktop), UI DEFERRED to EXCEL-032 (the desktop exposes indent only through the Format Cells dialog, which the web gains in EXCEL-032 — adding a standalone indent control now would EXCEED the desktop surface); built-in and custom number formats = read/write/persist already canonical (prior increments), import RENDER seeding now added (`n: {pattern}`); named cell styles = FAIL CLOSED/DEFERRED (no canonical engine path; `<cellStyles>` stays byte-preserved; "where supported" resolves to not supported); paste special + format painter = DEFERRED (clipboard semantics are not a style-delta family; no canonical path); diagonal borders = FAIL CLOSED (reader/writer never model them; the writer preserves diagonal edges + diagonalUp/Down attributes verbatim — proven in gateway tests and E2E).

Two defects found and fixed in the existing wire: (1) route `expectStyleEdit` DROPPED `borderTop: null` (and siblings) — a border clear never reached the engine, so the file's border silently survived the save (the same defect class as the EXCEL-026 freeze-clear fix); nulls now pass through as real clears with a dedicated test. (2) The route's indent bound was 0..15 while the writer, the OOXML schema, and the desktop all accept 0..250 — aligned to 0..250 with integer enforcement for both indent and textRotation (previously non-integers like 2.5 could reach the writer and produce schema-invalid attributes).

Coverage: gateway `xlsx-advanced-formatting.test.ts` 23/23 (reader: mixed styles/colors per side, single-edge + diagonal skip, all-four sides, unknown-style fail-closed, empty default, rotations 45/135/255, indent, out-of-domain 200 ignored; writer: all 13 line styles round-trip, color round-trip, one-side edit preserving other sides + diagonal + diagonalUp/Down verbatim, null-edge clear, all-side clear, unrelated-cell + untouched-sibling-sheet byte preservation; the combined fixture font+fill+numberFormat+alignment+border+rotation — border/rotation/bold edits each preserve every other family; rotation write/clear/vertical round-trips incl. the cell's own xf attribute check; indent write/clear; style-only edits never touch values; schema-order serialization with ARGB colors; border interning) + route `office-style-routes.test.ts` 11/11 (all 13 styles accepted on all four sides, null clears pass through, non-object edges / unknown keywords / malformed colors rejected, rotation 0..255 integer bounds, indent 0..250 bounds, combined payload, unknown-key seam) + web unit `cell-mutation-merge` +9 (bd four-edge mapping, engine clearBorder all-null mapping, one-edge isolation, diagonal + unknown-style fail-closed, all four rotation forms incl. {a:0}→0 and {a:-45}→135, indent px→steps + clamp, combined families in one mutation, merge with prior value edit; `ooxmlTextRotationToUniver` table) + web architecture guards +7 (no border/rotation OOXML or JSZip in apps/web/src; no borderStates/rotationStates/advancedFormatStates second family; journal-through-existing-style-family shape incl. the shared conversion tables; public-API-only entry points with no `as unknown as`/`_injector` in the new callbacks; import seeding parity; Ribbon wiring with no disabled stubs; snapshot-field read-back) + browser E2E `ribbon-advanced-formatting.spec.ts` 7 tests / all 14 mandated scenarios (import existing borders + rotation + indent + combined cell with live-model read-back through the public FRange facade; unmodelable "wave" border imports NOTHING; pixel-level render proof — clearing the imported border/rotation changes the rendered pixels; one-side edit with side isolation in the journal, the saved XML, and the reopened model incl. the engine's adjacent-edge clear semantics and the diagonal-preserving border entry; No Border clear journaled as four real nulls and persisted; rotation 45/-45/vertical round-trips with exact `textRotation="…"` XML assertions and the 0-clear dropping the attribute; the combined-format guard — a border edit on G1 preserves bold/fill/numberFormat/alignment/rotation and the other three edges through save/reopen; no-op preservation — a value-only save leaves styles.xml and the sibling sheet byte-identical with the wave border intact and every advanced format surviving the unrelated edit).
