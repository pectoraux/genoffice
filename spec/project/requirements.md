...
### The single canonical allocation kernel and the allocation authority (new scheduling export)

The time-phased resource answer is a CANONICAL scheduling result: `resourceAllocations(document, schedule)` in `packages/project-scheduling/src/allocation.ts` — a pure read-only projection of a GIVEN DerivedSchedule (never schedules, never mutates, never proposes commands; `schedule()` stays the sole authoritative scheduling operation, lock §6; leveling stays `levelResources`). SINGLE CANONICAL ALLOCATION KERNEL (the PROJECT-026 correction, per the Principal Architect's review of PR #28): the resource demand/capacity semantics — the demand-contribution rules (summary roll-ups, milestones, zero-duration tasks contribute nothing), the demand-interval construction from the canonical scheduled windows, the resource-calendar resolution, the capacity-window resolution (tightest covering window), the calendar-aware segmentation (zero demand while the resource is not working — never a false over-allocation — with segment boundaries at every assignment + capacity-window + working-period endpoint, midpoint evaluation), and the over-allocation predicate — live in exactly ONE implementation, `packages/project-scheduling/src/allocation-kernel.ts`, consumed by BOTH `resourceAllocations()` (which emits EVERY segment of the tiling, including zero-demand non-working segments, so the projection receives the authority's complete time-phased answer) and `levelResources()` (which collapses the consecutive over-allocated runs of the SAME segments into its conflict windows and leveling decisions — the leveler's PROJECT-013 observable behavior byte-identical, every L-golden passing unchanged). There is no second capacity engine anywhere — not in the renderer (the injected query, below) and not in the scheduling package itself: one kernel, two projections. Each segment carries `demandUnits`, `capacityUnits`, the canonical `overallocated` flag (the kernel's own conflict predicate), and the sorted active `assignmentIds`. The single-authority property is asserted BOTH ways: by test — the union of consecutive over-allocated segments equals the leveler's `overallocations` windows (start/finish, peak demand, tightest capacity, assignment set), including the resource-calendar clipping, the non-working split, and the availability-window segmentation scenarios — and structurally — the scheduling architecture suite fails if any module other than the kernel defines the shared allocation primitives, and fails if either consumer stops consuming the kernel (a reintroduced independent implementation in the leveler cannot pass the gate). Determinism is the shared kernel contract: allocations sorted by `resourceId` (locale-free code-unit), segments ascending, assignment ids sorted — invariant under task/assignment/resource array reordering and serialization round-trips.
...
Out of scope and NOT implemented: critical-path/resource GRID COLUMNS (the 022 grid surface is untouched this increment — the 025 precedent; the floats surface carries the values and hosts read `row.schedule` directly for cell display), resource leveling interactions (the `LevelResources` command surface is untouched; `levelResources`'s observable PROJECT-013 behavior is byte-identical — the correction only extracts the demand/capacity semantics it already implemented into the shared kernel it now consumes; the allocation itself is read-only and never proposes commands), resource USAGE tables / time-phased work per assignment (the `assignmentSchedules` projection already carries per-assignment work; a usage-table surface is later scope), slack EDITING (no commands exist for it), any host UI/Electron/web code (PROJECT-027/028 — the browser/desktop E2E lands with the shells, the accepted 021–025 precedent; this increment's evidence is the component + architecture-discipline suites over the packages plus test-layer integration against the REAL scheduling package), and reports (the work item's related-work column — the negative-slack and over-allocation signals are echoed for the reporting layer to interpret, never interpreted here). The work-item's "E2E + schedule assertions" is satisfied by: the canonical battery (`packages/project-scheduling/tests/project-026.test.ts` — the demand tiling incl. the calendar-aware zero-demand nights, the over-allocation flag, the capacity-window segment splits, the skip rules, determinism/purity/reordering invariance, and the LEVELER CROSS-CHECK proving the single-kernel equivalence — both consumers read the ONE shared canonical allocation kernel); the single-kernel segmentation-identity battery (the resource-calendar × availability intersection clipping, the non-working midday break splitting one over-allocation into TWO conflict windows, the two restrictive availability windows splitting the over-allocation at both capacity transitions, and the reordering invariance of BOTH consumers' kernel-driven outputs on the segmentation-heavy fixture); the unit batteries (`tests/critical-path.test.ts` — the verbatim echo contract, the slack geometry incl. the snapped-late-date and negative-slack cases, the both-endpoints link classification, windowing, degradation, determinism; `tests/resource-utilization.test.ts` — the verbatim-clipped band contract, the name echo, the degenerate/coded/uncoded degradation, the exactly-once evaluator consultation, determinism/purity); the integration battery (`tests/critical-resource-integration.test.ts` — the REAL `resourceAllocations` binding; float EQUALITY with the authority's own TaskSchedule values on the chain/branch/MFO fixtures; band EQUALITY with the authority's output clipped; the over-allocation windows equaling the REAL leveler's conflict record; the command → schedule → surface re-projection with byte-identical undo restoration; the scheduler-first degradation for a broken resource calendar; the additive pipeline with BOTH the calendar and resource inputs threaded; end-to-end 3× determinism); and the golden battery R01–R12 (`tests/critical-resource-goldens.test.ts` — the critical chain; the float branch with its slack bar; the impossible schedule (negative slack echoed verbatim); the zero-slack milestone; the mixed link classification; the calendar-aware demand tiling; the over-allocation; the capacity-window drop; viewport re-projection; the command/undo/redo tuple restoration; the absent-input degradation with byte-identical geometry; and the 3× byte-identical mixed command/viewport sequence). Determinism contract: every function is pure — the same inputs always produce byte-identical outputs (3×-repeat tested), no wall clock, no randomness, no `localeCompare` (discipline-scanned), inputs never mutated; the discipline suite additionally scans `critical-path.ts` for CPM/float computation markers (working-time arithmetic, graph traversal) and `resources.ts` for capacity-semantics markers (the capacity fields, the capacity-window data, assignment unit reads, the calendar evaluators) — the structural no-second-engine guards — the scheduling-side architecture suite enforces the SINGLE canonical allocation kernel (no module other than `allocation-kernel.ts` may define the shared allocation primitives; both `resourceAllocations()` and `levelResources()` must consume the kernel; a reintroduced independent implementation in the leveler fails the gate — negative-validated), and the public-surface lockstep carries the PROJECT-026 exports.

## PROJECT-027 — Electron shell

Establishes the native desktop host around the already-built shared renderer and engine — the point where the Project initiative becomes visibly testable as an APPLICATION rather than through package tests alone. The host stack is exactly the architecture-lock §3 app surface:

```text
Electron
   ↓ (typed contextBridge IPC — the only transport)
Project desktop host  (apps/project — @genoffice/project-desktop)
   ↓ (imports)
project-renderer-core
   ↓ (injected authorities + engine commands)
Project engine / scheduler / file adapters
```

**The host introduces NO scheduling, dependency, calendar, resource, or other Project semantics of its own.** Every semantic value it renders is a projection of the canonical authorities; every mutation is a semantic `ProjectCommand` built by the renderer-core builders and edit flows and applied through the session; every file crossing goes through the canonical file adapters. This is enforced STRUCTURALLY, not by convention — the desktop architecture suite (`apps/project/tests/unit/architecture.test.ts`) fails the build when a host module crosses any of these lines.

### The main process is native transport only

`src/main/**` owns window/lifecycle integration, the native application menu, the native open/save dialogs, raw filesystem reads/writes (size-capped), the single-instance/argv open path, and the unsaved-changes close-guard handshake. It imports NO `@genoffice/*` package and never sees a `ProjectDocument`: documents cross the process boundary as raw bytes and the adapters run renderer-side. Every menu item carries a `MenuCommandId` and forwards ACTIVATION ONLY to the renderer — the menu names no Project operation. Menu accelerators are DISPLAYED but deliberately UNREGISTERED (`registerAccelerator: false`): the renderer's keyboard translation is the single execution path for accelerator keys, so an active cell editor keeps its own keys (native text undo, caret movement, deletion) — a mid-edit menu fire is structurally impossible.

**One canonical bounded native read (correction round).** Every native file read — the open-picker path AND the argv/second-instance path — crosses ONE helper, `src/main/bounded-read.ts` (`boundedReadFile`): the file is stat'ed first so an oversized input is rejected WITHOUT a byte being read, then read through a single bounded window (stat size + 1, so a file that grew after the stat can never smuggle uncapped bytes across). The cap constant (`MAX_FILE_BYTES`, mirroring the canonical adapters' own input cap — defense in depth) is defined exactly once, in the helper. Read results are `NativeReadResult` VALUES on every surface (`{ ok: true, bytes }` or `{ ok: false, error }`): errors are never thrown, and the error variant carries NO bytes — the renderer can never receive uncapped file contents, structurally rather than conventionally (the discipline suite pins the single-helper routing, the absence of any raw Node `readFile` import in the host, the once-only cap definition, and fails if any read surface bypasses the helper — negative-validated).

**Single-instance lock ordering (correction round).** The single-instance lock is keyed on the userData path, so an isolated profile (`GENOFFICE_PROJECT_USER_DATA` — the E2E/scratch pattern) must be installed BEFORE `app.requestSingleInstanceLock()` is called: each scratch dir then carries its own isolated lock, and two launches with separate dirs run concurrently without deferring to each other or to a real install. The discipline suite pins the ordering (setPath strictly before the lock request); the desktop E2E proves the behavior end-to-end (a second instance sharing the dir exits without a window and forwards its argv document to the first; a second instance with an oversized document forwards it and the FIRST instance surfaces the transport cap error; two independent launches with separate scratch dirs both stay alive).

### The preload bridge

`src/preload/**` exposes exactly the typed `window.projectDesktop` surface (the shared `src/shared/ipc.ts` contract: pick-open/pick-save/read/write, confirm-discard, menu/close/open event subscriptions) through `contextBridge` — no other capability crosses. File READS cross as `NativeReadResult` values on both read surfaces (bytes only when `ok`, never uncapped — see the bounded-read invariant above). The renderer never touches `electron`, `ipcRenderer`, or Node APIs, which keeps the entire renderer binding unit-testable under jsdom with an in-memory fake of the same interface.

### The renderer binding

`src/renderer/bindings.ts` is the ONE module importing `@genoffice/project-scheduling`, wiring exactly the three canonical injection seams the accepted increments define: the `ScheduleRunner` (PROJECT-021) for the renderer session, the `CalendarWorkingTimeQuery` (`workingIntervals ∘ resolveCalendar`, PROJECT-025), and the `ResourceAllocationQuery` (`resourceAllocations`, PROJECT-026). The controller (`app.ts`) holds one `ProjectRendererSession`, one `ProjectViewState`, the projection + `buildGanttView` pipeline with the calendar/resource inputs threaded per render, open/save flows over the canonical adapters, and dirty tracking by DOCUMENT IDENTITY (sessions are immutable and undo restores the exact prior reference — the saved-document reference comparison is the honest dirty signal, with no false positives through undo/redo). The host's new-document template is document DATA (the standard Mon–Fri 09:00–17:00 week + a fixed Monday start), engine-validated and scheduler-derived — the host computes no dates.

### Real desktop rendering

`src/renderer/ui.ts` maps the pure fraction-space view models onto real DOM: the virtualized task grid and the synchronized Gantt timeline (time axis, bars with progress, milestone diamonds, dependency-link polylines, the PROJECT-025 calendar surfaces, the PROJECT-026 criticality/slack/resource-visualization surfaces), with `hitTestGantt` as the canonical geometry→entity inverse for pointer interaction and the persistent cell/link editor over the PROJECT-023/024 edit flows. Grid rows are KEYED and updated IN PLACE: a selection re-render must not replace the element under an in-flight double click (which would swallow the browser's synthetic click/dblclick); link activation uses host pointer double-press detection for the same reason. Pixel layout (row height, column widths, pane width, scroll) is host state per the accepted PROJECT-021/022 rules — never view state.

### Host keyboard/menu translation

`src/renderer/translate.ts` is the entire translation layer: two pure tables (`translateKeyDown`, `translateMenuCommand`) mapping native inputs to the accepted vocabulary — view intents through the reducer, document actions through the renderer-core builders, edit flows, session history, file actions. The menu table and the keyboard table CONVERGE on the same actions (the discipline suite pins the equality for every shared id).
...
- **`confirmDiscard`** — a three-button DOM dialog (Save / Don't Save / Cancel — `window.confirm` is two-button and cannot express the contract), with Escape-cancel and the dialog named after the project; the SAME shared-controller save/discard/cancel flow the desktop close guard runs. _(Superseded at PROJECT-030: the dialog is now the shared host binding's own `confirmUnsavedChanges` rendered in BOTH hosts — this bridge surface was removed; see §PROJECT-030.)_
- **`onCloseRequested` / `approveClose`** — the close-handshake registration surfaces. The beforeunload guard over them is PURELY SYNCHRONOUS: the browser's unload lifecycle cannot complete the controller's asynchronous save/discard/cancel handshake, so the guard NEVER initiates it — no close-handler call, no dialog, no async transaction mid-unload. It consults a registered DIRTY PROBE (the entry registers `() => app.dirty`) and, when the document is dirty, prevents the unload (plus the legacy `returnValue`) so the browser asks the user natively; a clean document leaves without any consultation. The registered close handler fires ONLY through the web bridge's in-app close request (`requestClose()` — the native window-close button's web analog, where the handshake CAN complete; no in-app affordance in the shared menu vocabulary closes the page today, so the seam is the contract registration's honest firing path). `approveClose` is a deliberate no-op: the browser owns the unload decision — no approval may bypass the dirty probe, and the page has no programmatic close for an approval to release. The controller's dialog flow continues to govern every in-app destructive action (New/Open over unsaved changes), and the native Electron close path keeps the full asynchronous controller handshake (Electron can await/coordinate its host event) — all unit- and E2E-proven.
...
## PROJECT-029 — Shared Project ribbon

Establishes the shared Project ribbon: the tabbed command surface BOTH hosts render — implemented ONCE in the shared host binding (`@genoffice/project-host`, the 028 extraction's point paying off), mounted by the shared DOM layer inside the app skeleton, identical on desktop and web BY CONSTRUCTION (the shells add zero ribbon code; each shell's discipline suite pins the no-private-ribbon rule). The work item's dependencies name the two surfaces the ribbon composes over: the shared renderer binding (PROJECT-021 — the controller/DOM/translation stack the ribbon rides) and selection/editing (PROJECT-023 — the task-selection state and the semantic command pipeline the ribbon's controls address).

### Structure and vocabulary (transport, not semantics)

The ribbon arranges the COMPLETE shared command vocabulary (`MENU_COMMAND_IDS`, all 15 commands — the vocabulary lockstep is pinned by the host discipline suite: the ribbon's `RIBBON_COMMAND_IDS` equal the menu vocabulary exactly, no more, no less) as tabs → groups → controls: _(PROJECT-030 later added `task.information` — the sixteenth id — under exactly the rule this section's scope discipline names: the command landed in the shared contract first and the menu/ribbon locksteps followed; see §PROJECT-030.)_

- **Task** (the default tab — the Project convention of leading with the semantic editing surface): _Tasks_ (New Task, Delete Task), _Schedule_ (Indent Task, Outdent Task), _History_ (Undo, Redo).
- **View**: _Zoom_ (Zoom In, Zoom Out, Fit to Project), _Outline_ (Collapse Selected, Expand Selected).
- **File**: _Document_ (New Project, Open Project…, Save, Save As…).

The ribbon introduces NO new command, no new semantic, no new dispatch: every control carries a `MenuCommandId` and activation forwards through the shared DOM layer's callback, which the controller wires to its existing `menuCommand` path — the SAME `translateMenuCommand` table the native menu, the DOM menu bar, and the keyboard path flow through. One translation, four input surfaces; the wiring (`onRibbonCommand: (command) => menuCommand(command)`) is pinned by the host architecture suite's source scan, and the unit suite proves the parity behaviorally (ribbon activation and menu activation produce identical documents through the shared table). The ribbon module (`src/ribbon.ts`) imports ONLY `./bridge.js` — the command vocabulary — and reads no contract, renderer-core, scheduling, or file surface (the exact-import scan).

### The four presentation echoes (and nothing else)

The ribbon reflects plain booleans the controller's update pipeline already computes — `canUndo`/`canRedo` (the session journal's honest flags: Undo/Redo enable/disable), `dirty` (the document-identity probe: the Save control's marker), and `hasSelection` (the view state's task selection: Delete/Indent/Outdent enable/disable). Echoes are PRESENTATION, never verdicts: a disabled control says "nothing to act on," while an enabled control with nothing selected surfaces the controller's status answer exactly like the same menu command (the controller remains the authority). Echo updates never touch the tab selection (ribbon-owned presentation state, the menu-bar dropdown precedent). Accelerators are DISPLAYED in tooltips (the menu bar's displayed forms) and execute nothing — the shared keyboard translation table remains the single execution path for accelerator keys, so an active cell editor keeps its own keys, and ribbon clicks mid-edit behave exactly like menu clicks mid-edit (the one dispatch path).

### Scope discipline and evidence

Out of scope and NOT implemented: new commands or vocabulary beyond the shared 15 (any future command lands in the shared contract first, and the ribbon's lockstep follows), icon/glyph assets and large-control presentations (the ribbon is labeled text buttons with accelerator tooltips over the deterministic shared stylesheet — visual richness is a later presentation increment), keyboard roving-tab traversal within the tab strip (the controls are native focusable buttons; the ARIA tab pattern's arrow-key navigation is a later accessibility increment), context-sensitive tab promotion (e.g. a Format tab appearing on selection — PROJECT-031 presentation-parity scope), and localization (labels are the shared English presentation, the menu bar's precedent). Determinism: the ribbon's DOM is a pure function of its build-time structure + the four echoes — two fresh instances render byte-identical chrome (unit-tested), the two-load E2E scenarios include the ribbon in their canonical-DOM comparisons, and the module carries no wall clock, randomness, or date computation (covered by the same generic discipline scans as every host module). Evidence: the host unit suite (structure, vocabulary lockstep, tab switching, activation forwarding, echo gating incl. the synthetic-event no-bypass proof, build determinism, and the REAL-controller integration — controls driving the real engine paths and save paths over the in-memory bridge), the shell no-private-ribbon guards, and the desktop E15 / web W16 E2E batteries driving the REAL applications (boot vocabulary + echoes; semantic controls through the real engine; Save through the real filesystem write / the real browser download; View controls through the real reducer) — the "desktop/web E2E" basis of the work item's verification, with the semantic controls' engine/save paths proven end-to-end in both hosts.

## PROJECT-030 — Shared dialogs

Establishes the shared dialog layer: the modal dialogs BOTH hosts render — implemented ONCE in the shared host binding (`@genoffice/project-host`, `src/dialogs.ts`), mounted on `document.body` (the modal-overlay convention), identical on desktop and web BY CONSTRUCTION (the shells lose their dialog code entirely; each shell's discipline suite pins the no-private-dialog rule). The work item's dependencies name the two surfaces the increment composes: the shared renderer binding (PROJECT-021 — the controller/DOM stack that owns the dialog lifecycle) and selection/editing (PROJECT-023 — the canonical editing model and commit flow the semantic dialog drives). Its DoD — "host-independent dialogs operate on commands" — lands as two facts below; its exclusion — "bespoke host-only semantics" — is enforced by the consolidation (the two bespoke pre-030 unsaved-changes dialogs are deleted, not supplemented).

### The unsaved-changes dialog: one implementation, zero transport

The three-button Save / Don't Save / Cancel dialog becomes the shared layer's `confirmUnsavedChanges(projectName)` — the SAME dialog the pre-030 web host rendered (Escape-cancel, focus-on-Save, settle-exactly-once, the `discard-*` testids), now rendered by the controller in BOTH hosts. The dialog leaves the host TRANSPORT: `confirmDiscard` is removed from `ProjectHostBridge`, from the desktop's self-contained IPC lockstep (the channel, the preload surface, and the main-process `dialog.showMessageBox` handler), and from the web bridge (its private `confirmDiscardDialog` and `.gp-web-dialog-*` chrome deleted). The desktop close-guard handshake is UNCHANGED — a prevented close still forwards `closeRequested` to the renderer, which consults the SHARED dialog and answers with `approveClose`; the dead-renderer auto-approve path stays; the native file PICKERS stay transport (an OS open/save picker is a file surface, not a decision dialog). The beforeunload boundary is untouched: the web guard remains purely synchronous (the dirty probe alone), and the in-app close request remains the one web firing path for the full save/discard/cancel handshake.

### The Task Information dialog: operates on commands

The shared command vocabulary grows to sixteen with `task.information` — the sanctioned path the 029 scope discipline named ("any future command lands in the shared contract first, and the ribbon's lockstep follows"): the native menu, the web menu bar, and the ribbon all carry the new id in lockstep, and the shared translation table maps it to the new `dialog` HostAction. No keyboard binding is invented for it (menu/ribbon activation are the firing surfaces — the keyboard table remains exactly the accepted vocabulary). The dialog opens for the focused/selected task (the indent/outdent target rule; no selection → the honest "No task selected" status, exactly like any semantic command with nothing to act on), ends any active cell edit (the dialog supersedes it), and initializes every field from the DISPLAYED values through the canonical draft protocol (`initialTaskEditDraft`) — the name verbatim, the duration as decimal working minutes, the start/finish as the schedule-first displayed instants (sliced ISO, read-only: the cell editor remains the surface for editing those — scope discipline).

OK commits each CHANGED field, in canonical order (name, then duration), through the SAME one-call canonical commit flow the cell editor runs — begin → draft → `commitTaskEditThroughSession` (semantic command → journal → reconcile → derived-timing refresh) — one field at a time, so the single-editor rule holds by construction and each accepted field is ONE semantic command (`RenameTask` / `SetTaskDuration`) with the engine as the single semantic validator. The dialog module itself imports NOTHING and builds no command: presentation in, draft strings out. A refused field (unparseable duration draft → the canonical invalid reason; engine rejection e.g. a negative duration → the engine's verbatim diagnostics) keeps the dialog open with the reason surfaced; already-applied fields stay applied (they were real commands — undoable; the fixed re-OK commits only the remaining delta). Cancel and Escape dispatch nothing. An unchanged-displayed OK dispatches nothing (`noChange` — no no-op command is ever journaled). The inherited cell-editor semantics are documented honestly: a duration draft initialized from a schedule-derived value that differs from the stored one still produces a real pin command — the identical behavior of the same edit through the grid.

### The modal rule and scope discipline

While the Task Information dialog is open it is MODAL to the app's command surface: menu/ribbon activation and the keyboard translation path are suspended (the menu bar's open-dropdown precedent), with two exceptions by design — re-firing `task.information` refreshes the dialog from the CURRENT document (the honest re-open: uncommitted drafts are discarded), and the HOST lifecycle events still run (a window close is never blockable by an in-page dialog — the full save/discard/cancel handshake may stack the shared unsaved-changes dialog on top, and only the topmost dialog answers the keyboard; an external open request argv/drag-drop loads the new document, which closes the task dialog — its target context is gone).

Out of scope and NOT implemented: start/finish editing in the dialog (the cell editor is the canonical surface for those fields — the dialog composes the name + duration pair with canonical commit semantics; more fields are a later increment), dependency/resource/constraint/percent-complete dialogs (later increments over their own command families), a keyboard accelerator for `task.information` (the keyboard table owns execution and binds no key), multi-task dialogs / tabbed dialog chrome (MS Project's multi-tab Task Information is a later presentation increment), non-modal dialogs, and localization (the shared English presentation, the menu bar's precedent). Determinism: both dialogs' DOM is a pure function of the input — no wall clock, no randomness, no date computation (display strings are sliced ISO); two fresh instances render byte-identical chrome (unit-tested); the dialog layer is covered by the same generic discipline scans as every host module plus its own zero-import guard. Evidence: the host unit suites (the dialogs module battery + the REAL-controller integration — the close-guard and file flows clicking the REAL shared dialog; the Task Information battery over the real engine/scheduler/journal), the shell no-private-dialog guards with the IPC-lockstep and vocabulary updates, and the desktop E10-rework/E16 + web W17 E2E batteries driving the REAL applications (the shared unsaved-changes dialog guarding real closes in both hosts; the Task Information dialog committing through the real engine with the scheduler's own derived dates asserted) — the "E2E" basis of the work item's verification.

## PROJECT-031 — Keyboard / navigation / menu parity

Establishes the navigation/editing/menu PARITY the work item names, delivered entirely through the accepted shared stack — the frozen hierarchy untouched (host physical input → the shared translation tables → renderer-core intents/state → the canonical `ProjectCommand`/session pipeline → engine + scheduler). No new semantic authority: the increment adds NO command, NO vocabulary id (the shared command set stays at sixteen), no scheduling surface, and no host-specific interaction layer — every addition is either renderer-core VIEW STATE (the interaction model PROJECT-021 owns), a shared translation-table row (the single execution path), shared PRESENTATION data (the menu/ribbon surfaces), or an echo-driven presentation transition.

### The focused cell (keyboard/mouse editing parity)

The view state's `TaskSelection` gains `focusField` — an `EditableTaskField` (taskName, duration, start, finish; the canonical `EDITABLE_TASK_FIELDS` order) making the FOCUSED CELL the pair `(focusId, focusField)`. Absent means the implicit `taskName`, so every pre-031 state and consumer is byte-compatible (the reducer, `reconcileViewState`, and all existing intents are unchanged in behavior; the back-compat proof is the full pre-031 suite passing unmodified). The new `moveCellFocus` intent (`next`/`previous`) walks the editable fields in canonical grid order, CLAMPED at both ends (deterministic no-ops, the `moveTaskFocus` discipline), bootstrapping exactly like `moveTaskFocus` (no focus → the first visible row, arriving on `taskName`); the move changes the COLUMN only (the orthogonal-axis rule: row focus, anchor, and selection are untouched). `reconcileViewState` drops a restored field outside the four and drops the field whenever its row is dropped (the cell is a PAIR — the same honest-pruning rule every other restored reference follows). The shared keyboard table maps the column keys — plain ArrowLeft/ArrowRight and Tab/Shift+Tab → `moveCellFocus` previous/next (Alt stays the outline gestures' modifier; Tab is bound PLAIN only — alt/ctrl variants belong to the host/OS); Enter/F2 become `beginEditFocusedCell`, editing the focused cell — the EXACT surface the mouse's cell activation reaches (the parity gap closed: the keyboard can now reach the duration/start/finish editors without a mouse). The editing gate is unchanged (Enter/Escape belong to the editor; every other key passes through to the input — the pinned 023 rule). A non-editable focused cell (a summary's scheduling values — the 023 editability rule) refuses through the reducer's reference-equal no-op, which the controller surfaces as the honest per-field status ("Duration is not editable on this row" — the "Cannot indent" precedent, never a silent nothing). The shared DOM layer renders the focused cell (`data-cell-focused` + a presentation-only highlight — an echo of view state, never a verdict); the focused-cell walk, the focused-cell edit, the refusal, and the keyboard/mouse equivalence (identical `TaskEditing` for the same cell reached either way) are unit-pinned over the REAL controller.

### The shared menu presentation (cross-host menu parity)

The ONE menu presentation table (`packages/project-host/src/menu-presentation.ts`, importing ONLY `./bridge.js` — the ribbon's exact-import discipline): the four sections (File → Edit → Task → View), every command's LABEL, and every displayed ACCELERATOR in the shared `Ctrl…` form — the single source the 028 scope note deferred here ("presentation-parity lockstep is PROJECT-031 scope"). The WEB menu bar renders it directly (the shell's local label/accelerator table is DELETED; the web discipline suite pins that no presentation literal survives in any shell source). The DESKTOP native menu mirrors it in `src/main/menu.ts` — the main process may not import `@genoffice/*` (the frozen native-transport boundary), so the mirror is pinned EQUAL by the desktop discipline suite: labels exactly, and each native accelerator equal to the shared display string under the ONE documented display→native rule (`Ctrl+` ⇔ `CmdOrCtrl+`; every other form — `Alt+…`, `Delete`, `Insert` — is identical in both conventions). The pre-031 cross-host display defect (desktop Redo `CmdOrCtrl+Shift+Z` vs web/ribbon `Ctrl+Y`) converges on the shared `Ctrl+Y` form — DISPLAY only; the shared keyboard table still binds both redo keys, so behavior is unchanged. The shared ribbon's tooltip accelerators read the same table (`menuAcceleratorFor` — the 029 rule "the menu bar's displayed forms", now one source instead of a hand-maintained copy); the ribbon keeps its own control LABELS (a ribbon is not a menu — the surfaces' label conventions differ legitimately). The displayed-accelerator honesty is pinned end-to-end: every displayed accelerator is a REAL binding of its command in the shared keyboard table, and the one command without a display (`task.information`) has no keyboard binding (menu/ribbon activation are the firing surfaces).

### The context-sensitive tab promotion (ribbon presentation parity)

The 029 scope discipline deferred "context-sensitive tab promotion" to this increment; it lands as an echo-driven presentation transition inside the shared ribbon: when the `hasSelection` echo crosses false → true (a selection APPEARS), the Task tab — the tab whose controls address the selection — is promoted (the Office contextual convention). No demotion fires when the selection disappears, and a manual tab choice holds while the context stays (promotion fires ONLY on the transition, so it never fights the user). This revises the 029 sentence "Echo updates never touch the tab selection": the echoes still never touch it EXCEPT through this one documented transition (the 029 spec sentence and its unit pins are updated in the same lockstep — the sanctioned scope note named 031).

### Scope discipline and evidence

Out of scope and NOT implemented: shortcuts beyond the accepted vocabulary (the work item's "unsupported shortcuts" exclusion — no PageUp/PageDown, no Ctrl+Home/End, no find/replace; the host never invents bindings for keys the accepted vocabulary does not name), cell-RANGE selection (the selection model stays row-scoped; columns have no extend semantic — shift is not consumed by the column walk), Tab-commits-and-advances inside the cell editor (the pinned 023 editor key-ownership rule stands: Enter/Escape only, every other key passes through to the input), roving-tab traversal within the ribbon tab strip (still the deferred accessibility increment the 029 scope note records), localization (the shared English presentation, the menu bar's precedent), and any new command/vocabulary (a future command lands in the shared contract first, and the menu/ribbon/presentation locksteps follow). Determinism: the presentation table is build-time constant data; the focused-cell walk is pure order arithmetic; the promotion is a pure transition function of the echo sequence — all covered by the same generic discipline scans as every host module. Evidence: the renderer-core suite (the `moveCellFocus` reducer battery + the focusField reconcile battery, 405 → 414), the host suite (the translation table's cell-key rows + the EVERY-displayed-accelerator-is-a-real-binding proof + the REAL-controller focused-cell battery — the walk, the focused-cell commit through the real engine/scheduler, the summary refusal, the keyboard/mouse equivalence; 126 → 140), the shell discipline suites (the web no-presentation-literal guard + the rendered-presentation pin; the desktop mirror-equality + display→native-rule pins; 29 → 31 / 43 → 45), and the desktop E17 / web W18 E2E batteries driving the REAL applications (the focused-cell keys walking the rendered cells; Enter committing through the real engine with the scheduler's own derived dates — the same canonical constants both hosts assert, the cross-host parity proof; the honest refusal; the promotion's transition rule; the real native menu / DOM menu bar presenting the shared table) — the "desktop/web E2E" basis of the work item's verification, with the navigation/editing parity the DoD names verified end-to-end in both hosts.
