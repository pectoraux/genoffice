...
The ribbon arranges the COMPLETE shared command vocabulary (`MENU_COMMAND_IDS`, all 15 commands — the vocabulary lockstep is pinned by the host discipline suite: the ribbon's `RIBBON_COMMAND_IDS` equal the menu vocabulary exactly, no more, no less) as tabs → groups → controls:
