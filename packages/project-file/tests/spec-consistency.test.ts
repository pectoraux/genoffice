 ...
 *   - PROJECT-025 remains the next authorized product increment, with all
 *     of its direct dependencies inside the accepted set.
 *
...
describe('specification consistency — work-items ↔ dependency-graph', () => {
  ...
  it('PROJECT-025 remains the next authorized product increment', () => {
    expect(frontierNext).toBe('PROJECT-025')
    ...
    expect(frontierAcceptedLast).toBe(24)
    ...
    expect(workItems.get('PROJECT-025')?.title).toBe('Calendar visualization')
    const depsFromTable = [...(workItems.get('PROJECT-025')?.deps ?? [])].sort()
    const depsFromGraph = [...(graphDeps.get('PROJECT-025') ?? [])].sort()
    expect(depsFromGraph).toEqual(depsFromTable)
    expect(depsFromTable).toEqual(['PROJECT-004', 'PROJECT-022'])
    for (const dep of depsFromTable) {
      expect(acceptedSet, `PROJECT-025 dependency ${dep} is not yet accepted`).toContain(dep)
    }
  })
