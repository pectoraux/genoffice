 ...
 *     (PROJECT-028 after the lockstep advance upon PROJECT-027's
...
describe('specification consistency — work-items ↔ dependency-graph', () => {
  ...
  it('PROJECT-028 remains the next authorized product increment', () => {
    expect(frontierNext).toBe('PROJECT-028')
    ...
    expect(frontierAcceptedLast).toBe(27)
    ...
    expect(workItems.get('PROJECT-028')?.title).toBe('Web shell')
    const depsFromTable = [...(workItems.get('PROJECT-028')?.deps ?? [])].sort()
    const depsFromGraph = [...(graphDeps.get('PROJECT-028') ?? [])].sort()
    ...
    for (const dep of depsFromTable) {
      expect(acceptedSet, `PROJECT-028 dependency ${dep} is not yet accepted`).toContain(dep)
    }
  })
