 ...
 * `@genoffice/project-engine` (the domain engine).
 *
 * Documents and enforces the clarified architecture-lock §13 rule (ACR-001):
 * the domain engine MUST NOT import `project-file`, MSPDI parser internals,
 * or MPP parser internals — the domain engine must not acquire file-format
 * knowledge. File-format implementations remain behind the `project-file`
 * adapter boundary; the engine's only runtime dependency is the contracts
 * package (the accepted dependency direction: engine → contracts).
 *
...
const ALLOWED_EXTERNAL = new Set(['@genoffice/project-contracts'])
const FORMAT_INTERNALS_RE = /mspdi|gproj|\bmpp\b|project-file/i

describe('PROJECT-engine architecture — the domain engine has no file-format knowledge', () => {
  it('ships exactly the contracts runtime dependency', () => {
    const pkg = JSON.parse(pkgRaw) as {
      dependencies?: Record<string, string>
      devDependencies?: Record<string, string>
    }
    expect(Object.keys(pkg.dependencies ?? {}).sort()).toEqual(['@genoffice/project-contracts'])
  ...
  it('never imports the scheduling, renderer, or host packages (dependency direction)', () => {
    for (const [file, source] of srcFiles) {
      const clean = stripComments(source)
      ...
      expect(clean, `${file} imports the scheduling package`).not.toMatch(
